            ...
            try
            {
                ...
                switch (method.ToLower())
                {
                    case "save":
                        ...
                        if (_filter != null)
                        {
                            if (!Utils.IsFloat(_filter[0]))
                                _filter[0] = "0";
                            if (!Utils.IsFloat(_filter[1]))
                                _filter[1] = "0";

                            float fromValue;
                            float toValue;
                            float.TryParse(_filter[0], out fromValue);
                            float.TryParse(_filter[1], out toValue);

                            if (fromValue > toValue)
                            {
                                responseMsg.Success = false;
                                responseMsg.Text = "Giá trị bắt đầu không thể lớn hơn giá trị kết thúc";
                                context.Response.Write(responseMsg.ToJsonString());
                                return;
                            }
