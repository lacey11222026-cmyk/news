...
namespace DATA.ContentDB
{
    ...
    {
        public static List<UserProject> SelectDynamicPage(string select, string where, string order, int CurrPage, int PageSize, ref int TotalRecord)
        {
            try
            {
                ...
                var list = new DBHelper(Configuration.HomeConnectionString).GetListSP<UserProject>("sp_UserProject_SelectPagedDynamic", pars);
        ...
        public static List<UserProject> SelectDynamic(string select, string where, string order)
        {
            try
            {
                ...
                var list = new DBHelper(Configuration.HomeConnectionString).GetListSP<UserProject>("sp_UserProject_SelectDynamic", pars);
        ...
        public static int UpdateView(int Id)
        {
            try
            {
                ...
                new DBHelper(Configuration.HomeConnectionString).ExecuteNonQuerySP("SP_UserProjectUpdateView", pars);
                return 1;
            }
            catch (Exception ex)
            {
        ...
        public static int InsertUpdate(UserProject functions)
        {
            try
            {
                ...
                new DBHelper(Configuration.HomeConnectionString).ExecuteNonQuerySP("sp_UserProject_InsertUpdate", pars);
        ...
        public static int Delete(int Id)
        {
            try
            {
                ...
                new DBHelper(Configuration.HomeConnectionString).ExecuteNonQuerySP("SP_UserProject_Delete", pars);
                return 1;
            }
            catch (Exception ex)
            {
        ...
        public static UserProject GetDetail(int DocumentId)
        {
            var select = "*";
        ...
        public static List<UserProject> GetSearch(int status, int type,string username, string keyword, int pageIndex, int pageSize, ref int totalRecords)
        {
            ...
            if (!string.IsNullOrEmpty(username))
            {
                if (!string.IsNullOrEmpty(where))
                    where += " AND ";
                where +=
                   "( Username =" + "'" + username + "' OR Id="+username +")" ;
            }
            if (type > 0)
            {
                if (!string.IsNullOrEmpty(where))
        ...
        public static List<UserProject> GetSearch2( int pageIndex, int pageSize, ref int totalRecords)
        {
            var select = "*";



            var where = "ViewCount=0 and Status=1";
            var orderBy = "Id DESC";

            //NLogLogger.DebugMessage(where);
            return SelectDynamicPage(select, where, orderBy, pageIndex, pageSize, ref totalRecords);
        }
        public static List<UserProject> TopProject(int top,string username,int status,string keyword)
        {
            var select = $"top ({top}) *";
