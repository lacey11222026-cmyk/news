    ...
    public class DocumentDBSproc: DocumentDBBase
    {
        public override int CreateUpdateDocument ( Document Document )
        {
            try
            {
                ...
                int _Agent = Document.Agent;
                int? _Area = Document.Area;
                int? _Type = Document.Type;
                int? _DocType = Document.DocType;

                using ( ShopOnlineDataContext datacontext = DataContext )
                    return datacontext.sp_Document_InsertUpdate(_id, _categoryid, _name, _categoryPathway, _description, _createdBy, _publishDate, _createdDate, _effectiveDate, _expiryDate, _status, _hits, _filepath, _code, _signedBy, _signedByDesc, _SiteId,_SiteUrl, _Area, _Agent, _Type, _DocType);
        ...
        public override IEnumerable<Document> GetDocumentsSearch(string keyword, int categoryId,int doctype, int status, int pageIndex, int pageSize, string fromdate, string todate, ref int totalRecords)
        {
            var select = "*";
        ...
        public override IEnumerable<Document> GetDocumentsSearch2(string keyword, string code, int agent, int area, int type, int categoryId, int doctype, int status, int pageIndex, int pageSize, string fromdate, string todate, ref int totalRecords)
        {
            var select = "*";
            ...
            {
                keyword = Utils.FormatKeywordSearch(code);
                where += "( Code LIKE N'%" + code + "%' ";
            ...
            if (!string.IsNullOrEmpty(fromdate))
            {
                ...
                if (!string.IsNullOrEmpty(where))
                    where += " AND ";
                where +=
                    " CONVERT(char(10), PublishDate,126) ='" + _FormDate.ToString("yyyy-MM-dd") + "' ";
            ...
            if (!string.IsNullOrEmpty(todate))
            {
                ...
                if (!string.IsNullOrEmpty(where))
                    where += " AND ";
                where +=
                    " CONVERT(char(10), EffectiveDate,126) ='" + _ToDate.ToString("yyyy-MM-dd") + "' ";
