...
namespace WebMVC4.Controllers
{
    public class DocumentController : Controller
    {
...

        {
            ...
            data = new DocumentBO().GetDocumentsSearchPaged2(keyword.Trim(), code.Trim(), agent, area, type, Id,1, 1, Page, PageSize, fromdate, todate, ref Total);
        ...
        [LocalizationActionFilter]
        public ActionResult Index2(int agent = 0, int area = 0, string keyword = "", int Page = 1)
        {
            ...
            var metaDescription = "Thủ tục hành chính";
            var siteTitle = "Thủ tục hành chính";
            var metaKeyword = siteTitle.Replace(" | ", ",");
            ViewBag.Description = metaDescription + " ";
            ViewBag.Keywords = metaKeyword + ConfigurationManager.AppSettings["DefMetaKeyword"];
            ViewBag.Title = siteTitle;
            ...
            var data = new List<DOCUMENT_FULL>();
            data = new DocumentBO().GetDocumentsSearchPaged2(keyword.Trim(), "", agent, area, -1, -1, 2, 1, Page, PageSize, "", "", ref Total);



            var Model = new DocumentModel { listdata = data, pageIndex = Page, pageSize = PageSize, total = Total };
        ...
        public ActionResult Detail2(int Id)
        {
            ...
            try
            {
                newsobj.Extend = JsonConvert.DeserializeObject<DocExtend>(newsobj.Description);
            }
            catch
            {

                newsobj.Extend = new DocExtend();
            }
