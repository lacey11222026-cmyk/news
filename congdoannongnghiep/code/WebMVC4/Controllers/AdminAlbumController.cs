    ...
    public class AdminAlbumController : Controller
    {
        ...
        public ActionResult GetAlbumDetail(int Id = 0)
        {
            ...
            if (Id > 0)
            {
                ...
                ViewBag.FileImage = GetAlbumImage(Id);
                model.AlbumImage = Newtonsoft.Json.JsonConvert.DeserializeObject<List<BIZ.Entity.AlbumImage>>(model.Images);
        ...
        public ActionResult SaveFileUpload(int id)
        {
            ...
            foreach (string fileName in Request.Files)
            {
                ...
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    strBuilder.Append(ConfigurationManager.AppSettings["UploadPath"]).Append("Album").Append("\\").Append(id / 100000).Append("\\").Append(id / 100).Append("\\").Append(id).Append("\\");
                    ...
                    {
                        fName = fileName1 + new Random().Next(10000) + extend1;
        ...
        public JsonResult SaveData(Album_FULL doc)
        {
        ...
        private List<SelectListItem> GetAlbumImage(int id)
        {
            ...
            // divided 1000000 files in folder               
            strBuilder.Append(ConfigurationManager.AppSettings["UploadPath"]).Append("Album").Append("\\").Append(Convert.ToInt32(id / 100000)).Append("\\").Append(Convert.ToInt32(id) / 100).Append("\\").Append(id).Append("\\");
