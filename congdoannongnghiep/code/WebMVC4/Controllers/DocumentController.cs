...
namespace WebMVC4.Controllers
{
    public class DocumentController : Controller
    {
        ...
        public ActionResult Index(int Id, string code="", string des="",string type="", string keyword="", int Page = 1)
        {
            ...
            data = new DocumentBO().GetDocumentsSearchPaged(keyword.Trim(), Id, 1, Page, PageSize, "", "", ref Total, code, des, type);
            ...
            ViewBag.code = code;
            ViewBag.SignedBy = type;
            ViewBag.SignedByDesc = des;
            if (string.IsNullOrEmpty(ViewBag.SignedByDesc))
                ViewBag.SignedByDesc = " ";
            if (string.IsNullOrEmpty(ViewBag.SignedBy))
                ViewBag.SignedBy = " ";
        ...
        public ActionResult ViewFile(int Id)
        {
            ...
            ViewBag.Keywords = ConfigurationManager.AppSettings["DefMetaKeyword"];
            ViewBag.Title = ConfigurationManager.AppSettings["DefMetaSiteTitle"];
            if (newsobj.FilePath.StartsWith("vanban"))
            {
                newsobj.FilePath = "https://media2.congdoannongnghiep.org.vn/Upload/" + newsobj.FilePath;
            }    
            ViewBag.FilePath = newsobj.FilePath;
            if (!newsobj.FilePath.Contains(".pdf")&& !newsobj.FilePath.Contains(".jpg"))
            {
