...
namespace BIZ
{
    public class DocumentBO
    {
        ...
        public List<DOCUMENT_FULL> GetDocumentsSearchPaged(string title, int categoryId, int status, int pageIndex, int pageSize, string fromdate, string todate, ref int totalRecords,string code="",string des="",string type="")
        {
            var Documents = DocumentDBBase.Create().GetDocumentsSearch(title, categoryId, status, pageIndex, pageSize,fromdate,todate, ref totalRecords,code,des,type);
            if (Documents == null)
                return null;
