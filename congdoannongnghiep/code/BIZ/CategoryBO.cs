    ...
    public class CategoryBO
    {
            ...
            if (isAdmin)
            {
                ...
                foreach (var item in lstcate)
                {
                    if (item.ParentId > 0)
                    {
                        ...
                        {
                            var pa = new CategoryBO().GetCategoryFull(item.ParentId.Value);
                            x1.Name = pa.Name + x1.Name;
                            listcategory.Add( x1);
                        }
                    }
                    else
        ...
        public List<CATEGORY_FULL> GetAllCategoriesFull(UTILS.Constants.CategoryType categoryType, string lang = "")
        {
            try
            {
                ...
                var strKeyCached = strGroupKeyCached + "_" + Constants.CACHE_KEY_ALL_CATEGORIES + (int)categoryType + lang;
                ...
                if (!string.IsNullOrEmpty(lang))
                    return lstItem.Where(x => x.Language == lang).ToList();
                return lstItem;
            }
            catch (Exception e)
            {
