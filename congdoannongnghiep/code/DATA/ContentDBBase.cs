...
namespace DATA
{
    public abstract class ContentDBBase : ShopOnlineDBBase
    {
        ...
        public abstract IEnumerable<Content> GetFilterContentsFrontend(int pageIndex, int pageSize, string title,
            int categoryId, ref int totalRecords, string fromdate, string todate, string lstNotId, string lang, int type,int chanelId);
        ...
        public abstract IEnumerable<Content>  GetFilterContents(int pageIndex, int pageSize, string title, int categoryId, List<int> lstcate, int status, string createdby, ref int totalRecords, string fromdate = "", string todate = "", string lststatus = "", string alias = "",int channelId=-1);

        public abstract IEnumerable<MarkST> GetSTContentMark(string title, int categoryId, int status, string createdby,
                                                             string fromdate = "", string todate = "");
        public abstract IEnumerable<Content> GetTopLastestContents(int top, int categoryId, int hot = 0);
