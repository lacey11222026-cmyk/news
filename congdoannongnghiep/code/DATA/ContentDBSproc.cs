...
namespace DATA
{
    public class ContentDBSproc : ContentDBBase
    {
        ...
        public override int CreateUpdateContent(Content content)
        {
            try
            {
                ...
                int? _hits = content.Hits;
                    int? _channelId = content.ChannelId;
                string _params = content.Params;

                using (ShopOnlineDataContext datacontext = DataContext)
                    return datacontext.sp_Content_InsertUpdate(_id, _categoryid, _title, _categoryPathway, _alias, _introtext, _contents, _image, _thumbnail, _url, _createdby, _createddate, _publishDate, _status, _type, _hits, _channelId,_params);
        ...
        public override IEnumerable<Content> GetFilterContentsFrontend(int pageIndex, int pageSize, string title, int categoryId, ref int totalRecords, string fromdate, string todate, string lstNotId, string lang, int type, int channelId)
        {
        ...
        public override IEnumerable<Content> GetFilterContents(int pageIndex, int pageSize, string title, int categoryId, List<int> lstcate, int status, string createdby, ref int totalRecords,string fromdate="",string todate="", string lststatus = "",string alias="", int channelId=-1)
        {
            title = Utils.FormatKeywordSearch(title);
            var select = "[Id],[CategoryId],[Title],[Alias],[IntroText],[Image],[Url],[CreatedBy],[CreatedDate],[PublishDate],[Status] ,[Type] ,[Mark] ,[Hits],ChannelId";
        ...
        public override IEnumerable<Content> GetTopContentByIds(string ids, int top)
        {
            ...
            var select = " Id,Title,PublishDate,IntroText,Image,Url,CategoryId,Type";
            if (top > 0)
                select = "TOP(" + top + ") Id,Title,PublishDate,IntroText,Image,Url,CategoryId,Type";
            var where = "Id IN (" + ids + ") AND Status = 4";
            var orderBy = "PublishDate DESC";

            return GetContentsDyn(select, where, orderBy);
        }
        public override IEnumerable<Content> GetTopLastestContents(int top, int categoryId,int hot=0)
        {
            var select = " Id,Title,PublishDate,IntroText,Image,Url,[Contents],[Type]";
            if (top >= 1)
                select = "TOP(" + top + ") Id,Title,PublishDate,IntroText,Image,Url,[Contents],[Type]";
            var where = "Status = 4";
            if (categoryId > 0)
            {
                if (!string.IsNullOrEmpty(where))
