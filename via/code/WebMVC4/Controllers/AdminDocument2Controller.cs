...
namespace WebMVC4.Controllers
{
    ...
    public class AdminDocument2Controller : Controller
    {
        ...
        public ActionResult ListDocument(int? cateId, int? status, string fromDate, string endDate, string title, int? currentPage, int? pageSize)
        {
            ...
            var data = new DocumentBO().GetDocumentsSearchPaged(title, CateId,1, Status, CurrPage, RecordPerPage, fromDate, endDate, ref TotalRecord);
            if (data.Count > 0)
        ...
        public ActionResult ListDocument2(int? cateId, int? status, string fromDate, string endDate, string title, int? currentPage, int? pageSize)
        {
            ...
            var data = new DocumentBO().GetDocumentsSearchPaged(title, CateId, 2, Status, CurrPage, RecordPerPage, fromDate, endDate, ref TotalRecord);
            if (data.Count > 0)
                ViewBag.TotalRecord = TotalRecord;
            else
                ViewBag.TotalRecord = 0;
        ...
        public ActionResult GetDocumentDetail2(int Id = 0)
        {
            
            var model = new DOCUMENT_FULL { Id = 0 };
            model.Extend = new DocExtend();
            ...
            if (Id > 0)
            {
                ...
                try
                {
                    model.Extend = JsonConvert.DeserializeObject<DocExtend>(model.Description);
                }
                catch
                {
                ...
                ViewBag.Title = "Cập nhật thủ tục hc";
            }
            else
            {
                ViewBag.Title = "Thêm mới thủ tục hc";
            }
            return View(model);
        }
        [ValidateAntiForgeryToken]
        [HttpPost]
        [ValidateInput(false)]
        public JsonResult SaveData2(DOCUMENT_FULL doc)
        {
            var ReturnData = new ReturnData();
            try
            {
                ...
                doc.DocType = 2;
                doc.CategoryId = 39;
                doc.CategoryPathway = ",39,";
                doc.Description = Utils.ConvertToJson(doc.Extend, string.Empty);
                IFormatProvider culture = new CultureInfo("en-US", true);
               ...
               var result = new DocumentBO().CreateUpdateDocument(doc);
                ...
                if (result >= 0)
                {
                    ...
                    {
                        ReturnData.Description = "Cập nhật Thành Công";
                        lognewsobj.Note = "Update thủ tục hc";
                    }

                    else
                    {
                        ReturnData.Description = "Thêm mới Thành Công";
                        lognewsobj.Note = "Tạo mới thủ tục hc";
                    }
