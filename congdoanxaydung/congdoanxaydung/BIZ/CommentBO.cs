...
namespace BIZ
{
    public class CommentBO
    {
        ...
        public List<Comment> GetCommentsPaged(string title, int type, long itemid, int status, int pageIndex, int pageSize, ref int totalRecords)
        {
            string keyCache = strGroupKeyCached + "_" + Constants.CACHE_KEY_CommentS_PAGED_JSON + pageIndex + "_" + pageSize + "_" + title + "_" + type + "_" + itemid + "_" + status;
            ...
            if (Comments.Count > 0)
            {
                RedisCaching.Add(keyCache, JsonConvert.SerializeObject(Comments));
            }
            return Comments;
        }
        ...
        public int PublishedComment(int id)
        {
            var returnVal = CommentDBBase.Create().PublishedComment(id);
            if (returnVal != -1)
                FlushAllCommentCache(strGroupKeyCached);
            return returnVal;
        }
