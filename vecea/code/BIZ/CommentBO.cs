    ...
    public class CommentBO
    {
        ...
        public Comment GetComment(int CommentId)
        {
            try
            {
                ...
                var item = (Comment)LocalCaching.GetData(strKeyCached);
                if (item != null)
                    return item;

                item = CommentDBBase.Create().GetComment(CommentId);
        ...
        public List<Comment> GetTopLastestComments(int top, int type = -1, long itemid = -1, int status = -1)
        {
            ...
            var lstItem = (List<Comment>)LocalCaching.GetData(strKeyCached);
