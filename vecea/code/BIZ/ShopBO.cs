...
namespace BIZ
{
    public class ShopBO
    {
        ...
        public List<Shop> GetShopsPaged(string keyword,int pageIndex, int pageSize, ref int totalRecords, int? published)
        {
            var data = ShopDBBase.Create().GetAllPaged(keyword,pageIndex, pageSize, ref  totalRecords, published);
            if (data == null)
                return null;
