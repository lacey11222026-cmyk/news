...
namespace WebMVC4.Controllers
{
    ...
    public class AdminCommentController : Controller
    {
        ...
        public ActionResult GetCommentDetail(int Id = 0)
        {
            var model = new Comment { Id = 0,  };
            if (Id > 0)
            {
                model = new CommentBO().GetComment(Id);
                if (model == null)
        ...
        [ValidateInput(false)]
        public JsonResult SaveData(Comment obj)
        {
            var ReturnData = new ReturnData();
            try
            {
                var model = new CommentBO().GetComment(int.Parse(obj.Id.ToString()));
                model.UserName = obj.UserName;
                model.Message = obj.Message;
                model.Published = obj.Published;
                var result = new CommentBO().CreateUpdateComment(model);
                ...
                if (result >= 0)
                {
                    ...
                    var lognewsobj = new ContentLog
                    {
                        ...
                        ItemId = int.Parse(obj.Id.ToString()),
                        ItemName = obj.ItemName,
                        Note = "Cập nhập bình luận",
                        Type = 1

                    };
                    ReturnData.Description = "Cập nhật Thành Công";
                    lognewsobj.Note = "Cập nhập bình luận";
