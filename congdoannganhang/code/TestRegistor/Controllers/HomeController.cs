...
namespace TestRegistor.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            if (Request.Cookies["testnameinfo2022"] != null)
            {
                var testname = Utils.Base64Decode(Request.Cookies["testnameinfo2022"].Value.ToString());
        ...
        public ActionResult Start(int id)
        {
            ...
            try
            {
                ...
                if (listarchiveInfo == null || listarchiveInfo.Count() < 3)
                {
                    ...
                    if(listQuestion.Exists(x=>x.Mark>3))
                    {
        ...
        public ActionResult Logout()
        {
            ...
            foreach (string cookie in HttpContext.Request.Cookies.AllKeys)
            {
                ...
                if (!cookie.ToLower().Contains("testnameinfo2022")) continue;
            ...
            if (Session["Mobile"] != null)
            {
                ...
                if (data != null)
                {
                    ...
                    if(data.Count>=1)
                    {
                        ...
                        ViewBag.Location = data.LastOrDefault().Location;
                        ViewBag.Role = data.LastOrDefault().Role;
                        ViewBag.Note = data.LastOrDefault().Note;
                    }    
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult LogOn(string Mobile, string FullName, string Location,string Role,string Note)
        {
            try
            {
                ...
                var testname = Utils.Base64Encode($"{Mobile}_{FullName}_{Location}_{Role}_{Note}");
                HttpCookie ck = new HttpCookie("testnameinfo2022", testname) { HttpOnly = true, Path = "/" };
