...
namespace DATA
{
    public class TestArchiveDBSproc: TestArchiveDBBase
    {
        ...
        public override IEnumerable<TestArchive> GetByRegistorId(int id, string mobile, int pageIndex, int pageSize, ref int totalRecords,int OrderType)
        {
            var select = "*";
            var where = "";
            ...
            if (!string.IsNullOrEmpty(mobile))
            {
                ...
                where += " ( Mobile="  + "'" + mobile + "' OR Location LIKE N'%" + mobile + "%' ) ";
            }
            var order = "Id DESC";
            ...
            if (OrderType == 2)
            {
                ...
                order = "Mark DESC,DATEDIFF ( millisecond , [StartTime] , [EndTime] )   ASC";
            }
