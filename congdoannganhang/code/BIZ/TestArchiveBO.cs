...
namespace BIZ
{
    public class TestArchiveBO
    {
        ...
        public List<TestArchive> GetByRegistorId(int id, string mobile, int pageIndex, int pageSize, ref int totalRecords, int OrderType)
        {
            var manuFactorys = TestArchiveDBBase.Create().GetByRegistorId(id, mobile, pageIndex, pageSize, ref totalRecords, OrderType);
            if (manuFactorys == null)
        ...
        public List<TestArchiveReport> Report(int id)
        {
            ...
            if (manuFactorys.Count > 0)
            {
                ...
                RedisCaching.Add(keyCache, JsonConvert.SerializeObject(manuFactorys), 60*5);
