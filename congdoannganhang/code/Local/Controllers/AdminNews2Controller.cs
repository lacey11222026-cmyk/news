...
{
    [Authorize(Roles = "Administrator,Local")]
    public class AdminNews2Controller : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            if (HttpContext.User.IsInRole("Administrator") || HttpContext.User.IsInRole("Local"))
        ...
        public ActionResult GetNewsDetail(int Id = 0)
        {
            ...
            var model = new CONTENT_FULL { PublishDate = DateTime.Now, Id = 0, Alias = HttpContext.User.Identity.Name, CategoryId = 0, Hits = 0, CreatedBy = HttpContext.User.Identity.Name, FileParam = new FileInfo(),Contents="" };
        ...
        public ActionResult SaveData(CONTENT_FULL newsobj)
        {
        ...
        public ActionResult UpdateStatus(string _id, string Title)
        {
            ...
            try
            {
                if (Id > 0)
                {
                    var obj = new ContentBO().GetContentFull(Id);
                    if (obj != null)
                    {
                        var lognewsobj = new ContentLog
                        {
                        ...
                        {
                            obj.Status = 2;
                            lognewsobj.Note = "Hạ bài viết";
                        }
                        new ContentBO().CreateUpdateContent(obj);
                        //Ghi log
                        Action<ContentLog> send = InsertContentLog;
                        var asynSend = send.BeginInvoke(lognewsobj, null, null);
