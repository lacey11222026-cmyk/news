...
namespace BIZ
{
    public class QABO
    {
        ...
        public List<QA> GetTopQA(int top)
        {
            var data = QADBBase.Create().GetTopLastest(top);
            if (data == null)
                return null;
        ...
        public List<QA> GetQAsPaged(int pageIndex, int pageSize, ref int totalRecords, int? published,string lang="")
        {
            var data = QADBBase.Create().GetAllPaged(pageIndex, pageSize, ref  totalRecords, published,lang);
            if (data == null)
                return null;
