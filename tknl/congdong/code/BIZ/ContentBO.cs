    ...
    public class ContentBO
    {
        ...
        public string GetFilterContentsPaged_JSON(int pageIndex, int pageSize, string title, int categoryId, int status, string createdby, string fromdate = "", string todate = "")
        {
            ...
            if (cachedata != null)
                return cachedata.ToString();
            int totalRecords = 0;
            var contents = new List<CONTENT_FULL>();
            if (categoryId >= 102)
            {
                var lstcate = new List<int>() { 29, 30, 45, 31, 46, 50 };
                contents = GetFilterContentFullsPaged(pageIndex, pageSize, title, -1, lstcate, status, createdby, ref totalRecords, fromdate, todate);
            }
            else
            {
                if (categoryId == 99)
                    categoryId = 0;
                contents = GetFilterContentFullsPaged(pageIndex, pageSize, title, categoryId, null, status, createdby, ref totalRecords, fromdate, todate);
            }
        ...
        private IEnumerable<Content> GetTopLastestContents(int top, int categoryId, string lang = "", int isHot = 0)
        {
            var result = ContentDBBase.Create().GetTopLastestContents(top, categoryId, lang, isHot);
            if (result == null)
                return null;
            return result;
        }
        ...
        public List<CONTENT_FULL> GetTopLastestContentFulls(int top, int categoryId = 0, int site = 0, string lang = "", int ishot = 0)
        {
            var strKeyCached = strGroupKeyCached + "_" + Constants.CACHE_KEY_TOP_LASTEST_CONTENTS + top + "_category" + categoryId + "_site" + site + "_lang" + lang + ishot;
            ...
            var lstItemBase = GetTopLastestContents(top, categoryId, lang, ishot);
            if (lstItemBase == null || lstItemBase.Count() == 0)
                return null;
