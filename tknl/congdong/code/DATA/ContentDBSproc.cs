...
namespace DATA
{
    public class ContentDBSproc : ContentDBBase
    {
        ...
        public override int CreateUpdateContent(Content content)
        {
            try
            {
                ...
                using (ShopOnlineDataContext datacontext = DataContext)
                    return datacontext.sp_Content_InsertUpdate(_id, _categoryid, _title, _categoryPathway, _alias, _introtext, _contents, _image, _thumbnail, _url, _createdby, _createddate, _publishDate, _status, _type, _hits, _params,IsHot,_SiteId);
        ...
        public override IEnumerable<Content> GetFilterContents(int pageIndex, int pageSize, string title, int categoryId, List<int> lstcate, int status, string createdby, ref int totalRecords,string fromdate="",string todate="", string lststatus = "",string alias="", string orderBy = "PublishDate DESC")
        {
            ...
            var select = "[Id],[CategoryId],[Title],[Alias],[IntroText],[Image],[Url],[CreatedBy],[CreatedDate],[PublishDate],[Status] ,[Type] ,[Mark] ,[Hits],[IsHot],[SiteId],[HitsAudio]";
            var where = string.Empty;
            //var orderBy = "Ordering ASC";
            //var orderBy = "PublishDate DESC";
            if (!string.IsNullOrEmpty(title))
                where += "(Title LIKE N'%" + title + "%' OR Thumbnail LIKE N'%" + title + "%' )";
            if (categoryId > 0)
            {
            ...
            else
            {
                if (lstcate != null)
                {
                    ...
                    where += " ( ";
                    foreach (var cateid in lstcate.Where(x=>x>0).Select((value, i) => new { value, i }))
                    {
                        if (cateid.value > 0)
                        {
                            if (cateid.i == 0)
        ...
        public override IEnumerable<Content> GetTopLastestContents(int top, int categoryId, string lang = "", int isHot = 0)
        {
