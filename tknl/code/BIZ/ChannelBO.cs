...
namespace BIZ
{
    public class ChannelBO
    {
        ...
        public Channel GetChannelFull(int channelId)
        {
         ...
         public List<Channel>  GetByContentId(int contentId)
         {
             var lstdata = new Channel_DataBO().GetByContentId(contentId);
             if (lstdata == null)
                 return null;
             string ids = "";
             ...
             ids = ids.TrimEnd(',');
             return GetChannelByIds(ids, 0, false);
         }
        public List<Channel> GetAllChannelsPaged(int pageIndex, int pageSize, ref int totalRecords)
        {
            var Channels = ChannelDBBase.Create().GetAllChannelsPaged(pageIndex, pageSize, ref totalRecords);
            if (Channels == null)
                return null;
        ...
        public List<Channel> GetAllChannelFullsPaged(int pageIndex, int pageSize, ref int totalRecords,int status)
        {
            string strKeyCached = Constants.CACHE_KEY_TOP_LASTEST_CHANNEL + pageIndex + "_pagesize" + pageSize+"_status"+status;
            string strKeyCachedTotal = Constants.CACHE_KEY_TOP_LASTEST_CHANNEL + pageIndex + "_pagesize" + pageSize + "_status" + status + "_total"; ;
            ...
            var lstItem = ChannelDBBase.Create().GetAllChannelsPaged(pageIndex, pageSize, ref totalRecords, status).ToList();
            if (lstItem.Count > 0)
            {
                LocalCaching.Add(strKeyCached, JsonConvert.SerializeObject(lstItem));
                LocalCaching.AddToGroupKey(strKeyCached, strGroupKeyCached);
