        ...
        */
        init: function (ed, url) {
            ...
            ed.addCommand('openPopup', function () {
                ...
                var button = {
                    "Chèn ảnh": function () {
                        //var timeupload = $("#timeupload").val().split('/');

                        //var imageUrl2 = IMAGE_UPLOAD_URL + EditorUploadUrl + '/' + timeupload[2] + '/' + timeupload[1] + '/' + timeupload[0] + '/';
                        var mceContent = '';
                        var width = '';
                        var height = '';
                        var align = '';
                        ...
                        if ($("#margin").val() != '') {
                            margin = "margin-top:" + $("#margin").val() + "px ;";
                            margin += "margin-bottom:" + $("#margin").val() + "px ;";
                        }
                        ...
                        if ($('#align option:selected').val() != '0') {
                            ...
                            if ($('#align option:selected').val() == 'center' || $('#align option:selected').val() == 'right' || $('#align option:selected').val() == 'left') {
                        ...
                        if ($('#align option:selected').val() == 'center') {
                            divalign = "data-mce-style=\"  margin-left: auto; margin-right: auto ;display: block" + ";text-align:center;width:" + $("#width").val() + ";" + "\"";
                            divstyle = "style=\"  margin-left: auto;margin-right: auto ;display: block" + ";text-align:center;width:" + $("#width").val() + ";" + "\"";

                        }
                        if ($('#align option:selected').val() == 'right' || $('#align option:selected').val() == 'left') {
                            divalign = "data-mce-style=\" float:" + $('#align option:selected').val() + ";text-align:center;" + "\"";
                            divstyle = "style=\" float:" + $('#align option:selected').val() + ";text-align:center;" + "\"";
                        }
                        $("#tinymce_ajaxupload_img_cont").find('input[name="image_name"]:checked').each(function (i, e) {
                            //imageUrl += "0.800.jpg";
                            mceContent += '<div style="max-width:475px;" class="news_detail_img" ' + divalign + ' ' + divstyle + '><img src="'+ $(e).prev().attr("src") + '"' + width + ' ' + height + ' ' + align + ' ' + style + '  /></div>';
                        });
            ...
            function _renderHtml() {
                ...
                html += ' Từ ngày: <input id="fromdate" style="width:80px;" />&nbsp;&nbsp;Đến ngày: <input id="todate" style="width:80px;" />&nbsp;&nbsp;Người tạo <select id="select_user" onchange="_loadImages();" name="select_category">&nbsp;&nbsp  </select> Từ khóa: <input id="title" style="width:100px;" />&nbsp;&nbsp;<button onclick="_loadImages();">Tra cứu</button>';
                ...
                html += 'Điều chỉnh&nbsp;&nbsp;<select id=\"align\" ><option value=\"0\">-- Not set --</option><option value=\"baseline\">Baseline</option><option value=\"top\">Top</option><option value=\"middle\">Middle</option><option value=\"bottom\">Bottom</option><option value=\"text-top\">Text top</option><option value=\"text-bottom\">Text bottom</option><option value=\"center\" selected>Center</option><option value=\"right\">Right</option><option value=\"left\">Left</option>	</select>&nbsp;';
                html += 'Kích thước ảnh&nbsp;&nbsp;<input id=\"width\" type=\"text\" maxlength=\"5\" style=\"width:50px\" size=\"3\" value=\"475\" name=\"width\">x<input id=\"height\" type=\"text\" maxlength=\"5\" style=\"width:50px\" size=\"3\" value=\"\" name=\"height\">';
                html += '&nbsp;Khảng cách&nbsp;&nbsp;<input id=\"margin\" type=\"text\" maxlength=\"5\" style=\"width:50px\"  size=\"3\" value=\"6\" name=\"margin\">' +
                   '&nbsp;<br><br></div>';
            ...
            var imageCurentUrl = IMAGE_UPLOAD_URL + EditorCurentUploadUrl + '/';

            function _loadData() {
                $.Services({

                    serviceName: 'User.ashx',
                    data: {
                        __m: 'get_all_users',
                    },
                    success: function (data) {
                        ...
                        if (data != undefined && data != null && data != '') {
                            ...
                            $.each(data, function (i, item) {
                                html = '<option value="' + item.Value + '" title="' + item.Text + '">';
                                html += item.Text;
                                html += '</option>';
                            ...
                            $('#select_user option[value=' + EditorCurUser + ']').attr('selected', 'selected');
                            _loadImages();
                        }
            ...
            function _loadImages() {
                ...
                var title = $("#title").val();
                var author = $('#select_user option:selected').val();
               // var imageUrl = IMAGE_UPLOAD_URL + EditorUploadUrl + '/' + timeupload[2] + '/' + timeupload[1] + '/' + timeupload[0] + '/';
                $.Services({
                    ...
                    success: function (data) {
                        if (data != undefined && data != null && data != '') {
                            ...
                            $.each(data, function (i, item) {
                                 ...
                                 html += '<img  height="100" src="' + item.FilePath + '"  />';
                                 html += '<input name="image_name" type="checkbox" value="' + item.Id + '"  />';
                            ...
                            $('#tinymce_ajaxupload_img_cont').html(html);
                        }
                        else {
                            $('#tinymce_ajaxupload_img_cont').html('');
                        }
            ...
            function _renderAjaxUpload(eid) {
                ...
                new AjaxUpload(eid, {
                    ...
                    onComplete: function (file, response) {
                        ...
                        //    var html = '<div style="float:left;margin:5px;">';
                        //    html += '<img height="100" src="' + imageCurentUrl + responseJson.Value + '"  />';
        ...
        */
        getInfo: function () {
            return {
                ...
                authorurl: 'www.tietkiemnangluong.vn',
                infourl: 'www.tietkiemnangluong.vn',
                version: "1.0"
            };
        }
