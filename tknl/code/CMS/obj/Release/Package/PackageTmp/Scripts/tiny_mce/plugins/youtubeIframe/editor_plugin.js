(function () {
    ...
    tinymce.create("tinymce.plugins.YoutubeIframePlugin", {
        init: function (a, b) {
            a.addCommand("mceYoutubeIframe", function () {
            ...
            a.addButton("youtubeIframe", {
                title: "youtubeIframe.desc",
                cmd: "mceYoutubeIframe",
                image: b + "/img/youtube.png"
            });
            a.onNodeChange.add(function (a, b, c) {
                var d = false;
                if (c.nodeName == "IMG") {
                    try {
                        var e = c.attributes["src"].value;
                        var f = c.attributes["alt"].value;
                        var g = e.match("vi/([^&#]*)/0.jpg");
                        d = g[1] === f
                    } catch (h) { }
                }
                b.setActive("youtubeIframe", d)
            })
        },
