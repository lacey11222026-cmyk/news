...
namespace DATA
{
    public abstract class ContentDBBase : ShopOnlineDBBase
    {
        ...
        public abstract IEnumerable<Content> GetAllContentsMarkPagedDyn(string select, string where, string orderBy,
                                                                        int pageIndex, int pageSize,
                                                                        ref int totalRecords, ref int totalMark);

        public abstract IEnumerable<Content> GetFilterContentsFrontend(int pageIndex, int pageSize, string title,
                                                                       int categoryId, ref int totalRecords,
                                                                       string fromdate, string todate, string lstId,string lang,int type,int ishot);
        public abstract IEnumerable<Content> GetFilterContents(int pageIndex, int pageSize, string title, int categoryId, List<int> lstcate, int status, string createdby, ref int totalRecords, string fromdate = "", string todate = "", string lststatus = "", int type = -1, string orderBy = "PublishDate DESC", int checkTime = 0);

        public abstract IEnumerable<Content> GetFilterContentsMark(int pageIndex, int pageSize, string title,
                                                                   int categoryId, int status, string createdby,
                                                                   ref int totalRecords, ref int totalMark,
        ...
        public abstract IEnumerable<MarkST> GetSTContentMark(string title, int categoryId, int status, string createdby,
                                                             string fromdate = "", string todate = "");
        public abstract IEnumerable<Content> GetTopLastestContents(int top, int categoryId,string lang,int IsHot,string title);
        public abstract IEnumerable<Content> GetTopViewContents(int top, int categoryId, string fromdate = "", string todate = "",string lang="");
