    ...
    public class DocumentDBSproc : DocumentDBBase
    {
        public override int CreateUpdateDocument(Document Document)
        {
            try
            {
                ...
                string _effectiveDate = Document.EffectiveDate;
                string _expiryDate = Document.ExpiryDate;
                int? _private = Document.Private;
                string _language = Document.Language;


                using (ShopOnlineDataContext datacontext = DataContext)
                    return datacontext.sp_Document_InsertUpdate(_id, _categoryid, _name, _categoryPathway, _description, _createdBy, _publishDate, _createdDate, _effectiveDate, _expiryDate, _status, _hits, _private, _filepath, _code, _signedBy, _language,_signedByDesc);

            }
            catch (Exception exp)
            {
                ExHandler.Handle(exp, "DocumentDBSproc", "CreateUpdateDocument");
                return -1;
            }
        }
        ...
        public override IEnumerable<Document> GetDocumentsDyn(string select, string where, string orderBy)
        {
            ...
            {
                ExHandler.Handle(exp, "DocumentDBSproc", "GetDocumentsDyn: select" + select);
                return null;
            }
        }
        ...
        public override IEnumerable<Document> GetAllDocumentsPagedDyn(string select, string where, string orderBy, int pageIndex, int pageSize, ref int totalRecords)
        {
            ...
            {
                ExHandler.Handle(exp, "DocumentDBSproc", "GetAllDocumentsPagedDyn: select" + select);
                return null;
            }
        }
        ...
        public override int DeleteDocumentDyn(string where)
        {
            ...
            {
                ExHandler.Handle(exp, "DeleteDocumentDyn " + where);
                return -1;
            }
        }
