    ...
    public class BannerDBSproc: BannerDBBase
    {
        public override int CreateUpdateBanner ( Banner Banner )
        {
            try
            {
                ...
                int? _site = Banner.Site;
                int? _categoryId = Banner.CategoryId;
                

                using ( ShopOnlineDataContext datacontext = DataContext )
                    return datacontext.sp_Banner_InsertUpdate(_id, _name, _data, _description, _region, _status, _type, _url, _oder, _site, _categoryId);
        ...
        public override IEnumerable<Banner> GetTopLastestBanners(int top, int region,int status,int site=0,int categoryId=0)
        {
            ...
            if (site > -1)
            {
                ...
                where += " Site=" + site.ToString();
            }
            var orderBy = "[Order] ASC, [ModifyDate] DESC";
