...
function GetAJAXPaging(totalRecords, pagesize, pageIndex, ajax, staticserver) {
    pageIndex = parseInt(pageIndex.toString());
    pagesize = parseInt(pagesize.toString());
    totalRecords = parseInt(totalRecords.toString());
    var totalNumericLinks = 10; // Số link hiển thị
    ...
    if (totalNumericLinks >= totalPages) {
        ...
        if (pageIndex - totalNumericLinks / 2 > 0) {
            ep = (pageIndex + totalNumericLinks / 2 - (totalNumericLinks - 1) % 2);
    ...
    var FirstItemIndex = pagesize * (pageIndex - 1) + 1;
    var LastItemIndex = pagesize * pageIndex;
    var str = '';
    str += '<div class="phantrang"><ul>';
   
    var prev = pageIndex - 1;
    var next = pageIndex + 1;

    if (pageIndex > 1) {
        str += '<li><a  class="bt_prev" href="JavaScript:void(0);" onclick="return ' + ajax + prev + ')"> < </a></li>';
    ...
    for (var i = sp; i <= ep; i++) {
        ...
        if (i != pageIndex) {
            if (i == ep)
                str += '<li><a  href="JavaScript:void(0);" onclick="return ' + ajax + i + ')">' + i + '</a</li>';
            else
                str += '<li><a  href="JavaScript:void(0);" onclick="return ' + ajax + i + ')">' + i + '</a></li>';
        }
            ...
            if (i == ep)
                str += '<li><a  class="fix" href="#" >' + i + '</a></li>';
            else
                str += '<li><a  class="fix" href="#" >' + i + '</a></li>';
        }
    }
    // thêm nút xem trang sau và trang cuối
    if (pageIndex < totalPages) {
        str += '<li><a  class="bt_next" href="JavaScript:void(0);" onclick="return ' + ajax + next + ')"> > </a></li>';

    }


    return str + '</ul><div style="clear: both"> </div></div></div>';
