...
namespace WebMVC4.Controllers
{
    ...
    public class AdminTestController : Controller
    {
        ...
        public ActionResult ListTestArchive(string keyword, int registorId, int? orderType, int? currentPage, int? pageSize,int?status)
        {
            ...
            int Status = status == null ? -1 : (int)status;
            var data = new TestArchiveBO().GetByRegistorId(registorId, keyword, CurrPage, RecordPerPage, ref TotalRecord, OrderType, Status);
            if (data.Count > 0)
                ViewBag.TotalRecord = TotalRecord;
            else
                ViewBag.TotalRecord = 0;
