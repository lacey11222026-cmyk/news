...
namespace WebMVC4.Controllers
{
    ...
    public class AdminNews2Controller : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            if (HttpContext.User.IsInRole("Administrator") || HttpContext.User.IsInRole("NewsCreate"))
        ...
        public ActionResult ListNews(int? cateId, int? status, string fromDate, string endDate, string title, string createdBy, int? currentPage, int? pageSize, string orderBy = "PublishDate DESC")
        {
            ...
            if (ViewBag.IsNewsPublish)
            {
                ...
                data = new ContentBO().GetFilterContentFullsPaged(CurrPage, RecordPerPage, title, CateId, null, Status, createdBy, ref TotalRecord, -1, fromDate, endDate, lststatus, orderBy);
            }
            else
            {
                ...
                {
                    lststatus = "1,2,4";
                    data = new ContentBO().GetFilterContentFullsPaged(CurrPage, RecordPerPage, title, CateId, lstCate, Status, createdBy, ref TotalRecord, -1, fromDate, endDate, lststatus, orderBy);
                }
                else
                {
                    lststatus = "1,3,4";
        ...
        public ActionResult GetNewsDetail(int Id = 0)
        {
            ...
            var model = new CONTENT_FULL { PublishDate = DateTime.Now, Id = 0, Album = HttpContext.User.Identity.Name, CategoryId = 0, Hits = 0, CreatedBy = HttpContext.User.Identity.Name, Contents = " " };
...

        {
            ...
            var model = new CONTENT_FULL { PublishDate = DateTime.Now, Id = 0, Album = HttpContext.User.Identity.Name, CategoryId = 55, Hits = 0, CreatedBy = HttpContext.User.Identity.Name, Contents = " ", Type = (byte)Temp, IntroText = " " };
        ...
        public ActionResult SaveData(CONTENT_FULL newsobj, string actionType, string note)
        {
            ...
            switch (actionType.ToLower())
            {
                case "add":
                    ...
                    lognewsobj.Description = "Tạo mới bài viết";
                    newsobj.Status = 4;
                    ...
                    else
                    {
                        if (ViewBag.IsNewsEdit)
                        {
                            newsobj.Status = 4;//đang biên tập
                        }
                        else
                        {
                ...
                case "reject":
                    if (ViewBag.IsNewsPublish)
                    {
                        if (newsobj.Status == 1)
                        {
                            ...
                            //lấy tài khoản xuất bản+admin+thằng viết
                            //.Role = GetUserByNewsRole(newsobj.CreatedBy);
                        }
                        else
                        {
                ...
                case "send":
                    ...
                    if (ViewBag.IsNewsEdit)
                    {
                        lognewsobj.Description = "Gửi xuất bản";
                        newsobj.Status = 2;
                        // lấy tài khoản xuất bản+admin
                        //notiobj.Role = GetUserByNewsRole("");
                ...
                case "save":
                    ...
                    lognewsobj.Description = "Lưu bài";
                    //lấy tài khoản xuất bản+admin+thằng viết
                    //notiobj.Role = GetUserByNewsRole(newsobj.CreatedBy);
            ...
            newsobj.CategoryPathway = cateobj.Pathway;
            newsobj.Language = cateobj.Language;
            newsobj.CreatedRole = "news";
            //update news
