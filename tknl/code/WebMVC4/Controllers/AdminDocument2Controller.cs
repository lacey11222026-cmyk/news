...
namespace WebMVC4.Controllers
{
    ...
    public class AdminDocument2Controller : Controller
    {
        ...
        public ActionResult ListDocument(int? cateId, int? status, string fromDate, string endDate, string title, int? currentPage, int? pageSize)
        {
            ...
            var data = new DocumentBO().GetDocumentsSearchPaged(title, CateId, Status, CurrPage, RecordPerPage, fromDate, endDate, ref TotalRecord);
            if (data.Count > 0)
        ...
        {
            ViewBag.CategoryList = _staticCategoryList;
            var model = new DOCUMENT_FULL { Id = 0,PublishDate=DateTime.Now };
            if (Id > 0)
            {
            ...
            try
            {
                ...
                if (result >= 0)
                {
                    ...
                    if (doc.Id > 0)
                    {
                        ...
                        lognewsobj.Description= "Update văn bản";
                    }

                    else
                    {
                        ReturnData.Description = "Thêm mới Thành Công";
                        lognewsobj.Description= "Tạo mới văn bản";
                    }
            ...
            try
            {
                if (Id > 0)
                {
                    ...
                    if (obj != null)
                    {
                        ...
                        {
                            obj.Status = 0;
                            lognewsobj.Description= "Khóa văn bản";
                        }
