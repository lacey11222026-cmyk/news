...
namespace WebMVC4.Controllers
{
    ...
    public class AdminNews2Controller : Controller
    {
        ...
        public ActionResult ListCrawl(int Type,string title)
        {
            var listdata = ServerProcess.GetTopNews(Type, 50, title);
            ViewBag.Type = Type;
            return PartialView(listdata);
        }
        public ActionResult DetailCrawl(int Id,int Type)
        {
            ViewBag.IsNewsEdit = false;
            ViewBag.IsNewsPublish = false;
            if (HttpContext.User.IsInRole("Administrator") || HttpContext.User.IsInRole("NewsPublish")|| HttpContext.User.IsInRole("Comment"))
                ViewBag.IsNewsPublish = true;
            if (HttpContext.User.IsInRole("Administrator") || HttpContext.User.IsInRole("NewsEdit") || HttpContext.User.IsInRole("Comment"))
                ViewBag.IsNewsEdit = true;

            if (HttpContext.User.IsInRole("Administrator") || HttpContext.User.IsInRole("Guest") || HttpContext.User.IsInRole("Comment"))
                ViewBag.IsNewsCreate = true;
            ...
            var model = ServerProcess.GetDetail(Type, Id);
            model.Id = 0;
            try
            {
            ...
            if (model == null)
                return RedirectToAction("Crawl");
            ViewBag.CategoryList = listcategory;
            return View("GetNewsDetail", model);
        }
        ...
        public ActionResult ListNews(int? cateId, int? status, string fromDate, string endDate, string title, string createdBy, int? currentPage, int? pageSize, string orderBy = "PublishDate DESC")
        {
            ...
            if (ViewBag.IsNewsPublish)
            {
                ...
                data = new ContentBO().GetFilterContentFullsPaged(CurrPage, RecordPerPage, title, CateId, null, Status, createdBy, ref TotalRecord, fromDate, endDate, lststatus, "", orderBy);
            }
            else
            {
                if (ViewBag.IsNewsEdit)
                {
                    lststatus = "1,2,4";
                    data = new ContentBO().GetFilterContentFullsPaged(CurrPage, RecordPerPage, title, CateId, lstCate, Status, createdBy, ref TotalRecord, fromDate, endDate, lststatus, "", orderBy);
                }
                else
                {
                    lststatus = "1,3,4";
        ...
        public ActionResult ListNews2(int? cateId, int? status, string fromDate, string endDate, string title, string createdBy, int? currentPage, int? pageSize, int? channelId, string orderBy = "PublishDate DESC")
        {
            //ViewBag.Createdby = HttpContext.User.Identity.Name;
            ViewBag.IsNewsEdit = false;
            ViewBag.IsNewsPublish = false;
        ...
        public ActionResult GetENewsDetail(int Id = 0, int Temp = 1)
        {
            ...
            var model = new CONTENT_FULL { PublishDate = DateTime.Now, Id = 0, Alias = HttpContext.User.Identity.Name, CategoryId = 7, Hits = 0, CreatedBy = HttpContext.User.Identity.Name, FileParam = new FileInfo(), Type = (byte)Temp };
            ViewBag.UserName = HttpContext.User.Identity.Name;
            var lstdata = Membership.GetAllUsers();
            //ExHandler.Handle(new Exception(), "User", "User" + lstdata.Count);
            List<AccountInfo> lstuser = new List<AccountInfo>();
            ...
            else
            {
                ...
                int categoryId = 8;
                categoryId += Temp;
                model.Contents = new CategoryBO().GetCategoryFull(categoryId).Contents;
                //listcategory = listcategory.Where(x => x.Published == 1).ToList();
            }
            ViewBag.CategoryList = listcategory;
            return View(model);
        ...
        public ActionResult SaveData(CONTENT_FULL newsobj, string actionType, string note)
        {
            ...
            switch (actionType.ToLower())
            {
                ...
                case "reject":
                    ...
                    {
                        lognewsobj.Note = "Trả lại phóng viên";
                        newsobj.Status = 3;//đang viết
                        notiobj.Title = String.Format("<b>{1}</b> trả lại phóng viên bài viết <b>{0}</b>", newsobj.Title, HttpContext.User.Identity.Name);
                        //lấy tài khoản xuất bản+admin+thằng viết
                        notiobj.Role = GetUserByNewsRole(newsobj.CreatedBy);
                    }
                ...
                case "send":
                    ...
                    {
                        lognewsobj.Note = "Gửi biên tập";
                        newsobj.Status = 1;//đang biên tập
                        // lấy tài khoản xuất bản+admin
                        notiobj.Role = GetUserByNewsRole("");
                        notiobj.Title = String.Format("<b>{1}</b> gửi phóng viên bài viết <b>{0}</b>", newsobj.Title, HttpContext.User.Identity.Name);
                    }
        ...
        private bool CheckPermission(CONTENT_FULL data, bool IsNewsPublish, bool IsNewsEdit, string Currentcreatedby)
        {
            if (IsNewsPublish)
            {
                ...
                if (data.Status == 3 && data.CreatedBy == Currentcreatedby)
                {
