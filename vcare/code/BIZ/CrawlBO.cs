...
namespace BIZ
{
    public class CrawlBO
    {
        ...
        public string getLastbyclass(string id, string dom, string html)
        {
            try
            {
                HtmlDocument doc = new HtmlDocument();
                ...
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    var lst = doc.DocumentNode.SelectNodes("//" + dom + "[@class='" + id + "']").ToList();
                    lst.Reverse();
                    foreach (HtmlNode selectNode in lst)
                    {
                ...
                {
                    var lst = doc.DocumentNode.SelectNodes("//" + dom).ToList();
                    lst.Reverse();
                    foreach (HtmlNode selectNode in lst)
                    {
                        return selectNode.InnerHtml;
        ...
        public string getmetaTag(string property, int index, string html)
        {
            try
            {
                HtmlDocument doc = new HtmlDocument();
                doc.LoadHtml(html);
                var selectNode = doc.DocumentNode.SelectNodes("//meta[@property='" + property + "']");
                if (selectNode.Count - 1 >= index)
                    return selectNode[index].Attributes["content"].Value;
                return "";
            }
            catch
            {
