    ...
    public class ProductDBSproc : ProductDBBase
    {
        ...
        public override int CreateUpdateProduct(Product manufactory)
        {
            ...
            try
            {
                ...
                string _national = manufactory.Url;
                int? _waterType = manufactory.Size;
                string _flag = manufactory.Volumn;
                DateTime? _time = manufactory.UpdateTime;
                using (ShopOnlineDataContext datacontext = DataContext)
                {
                    datacontext.SP_Product_InsertUpdate(_id, _name, _description, _intro, _tech, _national, _image, _album, _config, _ishot, _isnew, _price, _priceReal, _availableSell, _status, _service, _waterType, _flag, _order, _manufactoryId, _cateId, _catepath, _time, ref responecode);
        ...
        public override Product GetByUrl(string url)
        {
            var select = "*";
            var where = "Url = '" + url +"'";
        ...
        public override IEnumerable<Product> GetTopLastest(int top, int categoryId, int manufactoryId, int size,int hot,int id, decimal price, int notManuId)
        {
            var select = " *";
            if (top > 1)
                select = "TOP(" + top + ") *";
            ...
            //    where += "Price <= " + price;
            //}
            var orderBy = "[UpdateTime] DESC, ID DESC";

            return GetProductsDyn(select, where, orderBy);
        }
        public override IEnumerable<Product> GetAllPaged(string keyword, int categoryId, int manufactoryId,string manufactory,int s, int v, int pageIndex, int pageSize, ref int totalRecords, int? published, bool? isHot, bool? isNew, int model = 0, int min = 0, int max = 0, int orderType = 0, int carId = 0)
        {
            ...
            if (!string.IsNullOrEmpty(manufactory))
            {
                ...
                if (manufactory.EndsWith(","))
                    manufactory = manufactory.Remove(manufactory.Length - 1);
                if (manufactory.StartsWith(","))
                    manufactory = manufactory.Substring(1, manufactory.Length - 1);
                where += "ManufactoryId IN (" + manufactory + ")";
            ...
            if (orderType > 0)
            {
                switch (orderType)
                {
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        orderBy = "[IsHot] DESC,[UpdateTime] DESC, ID DESC";
                        break;
                }
            }
