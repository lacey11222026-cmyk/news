    ...
    public class ContentDBSproc : ContentDBBase
    {
        ...
        public override int CreateUpdateContent(Content content)
        {
            try
            {
                ...
                string _SiteUrl = content.SiteUrl;
                using (ShopOnlineDataContext datacontext = DataContext)
                    return datacontext.sp_Content_InsertUpdate(_id, _categoryid, _title, _categoryPathway, _alias, _introtext, _contents, _image, _thumbnail, _url, _createdby, _createddate, _publishDate, _status, _type, _hits, _params, IsHot, _SiteId, _SiteUrl);
        ...
        public override IEnumerable<Content> GetFilterContents(int pageIndex, int pageSize, string title, int categoryId, List<int> lstcate, int status, string createdby, ref int totalRecords,string fromdate="",string todate="", string lststatus = "",string alias="", string orderBy = "PublishDate DESC",string lang="")
        {
            title = Utils.FormatKeywordSearch(title);
            var select = "[Id],[CategoryId],[Title],[Alias],[IntroText],[Image],[Url],[CreatedBy],[CreatedDate],[PublishDate],[Status] ,[Type] ,[Mark] ,[Hits],[IsHot],[SiteId]";
        ...
        public override IEnumerable<Content> GetFilterContentsFrontend(int pageIndex, int pageSize, string title, int categoryId, ref int totalRecords, string fromdate, string todate, string lstNotId, string lang, int type,int site)
        {
            title = Utils.FormatKeywordSearch(title);
            var orderBy = "PublishDate DESC";
            var select = " Id,Title,PublishDate,IntroText,Image,Url,CategoryId,Hits,SiteId,SiteUrl,Alias,Thumbnail";
            var where = "Status=4 ";
            //var orderBy = "Ordering ASC";
            //var orderBy = "PublishDate DESC";
            if (!string.IsNullOrEmpty(title))
                where += "AND (Title LIKE N'%" + title + "%'  OR [Contents] LIKE N'%" + title + "%'  OR SiteUrl LIKE N'%" + title + "%')";
            if (categoryId > 0)
            {
        ...
        public override IEnumerable<Content> GetTopContentByIds(string ids, int top)
        {
            ...
            var select = " Id,Title,PublishDate,IntroText,Image,Url,CategoryId,SiteId,SiteUrl,Alias";
            if (top > 0)
                select = "TOP(" + top + ") Id,Title,PublishDate,IntroText,Image,Url,CategoryId,Alias";
            var where = "Id IN (" + ids + ") AND Status = 4";
            var orderBy = "PublishDate DESC";

            return GetContentsDyn(select, where, orderBy);
        }
        public override IEnumerable<Content> GetTopLastestContents(int top, int categoryId,string site,string lang)
        {
            var select = " Id,Title,PublishDate,CreatedDate,IntroText,Image,Url,[Contents],[Type],SiteId,SiteUrl,CategoryId,Alias";
            if (top >= 1)
                select = "TOP(" + top + ") Id,Title,CreatedDate,PublishDate,IntroText,Image,Url,[Contents],[Type],SiteId,SiteUrl,CategoryId,Alias";
            var where = "Status = 4";
            if (categoryId > 0)
            {
                if (!string.IsNullOrEmpty(where))
        ...
        public override IEnumerable<Content> GetTopViewContents(int top, int categoryId,string lang, string title, string fromdate, string todate)
        {
            title = Utils.FormatKeywordSearch(title);
            var select = " Id,Title,PublishDate,IntroText,Image,Url,SiteId,SiteUrl";
            if (top >= 1)
