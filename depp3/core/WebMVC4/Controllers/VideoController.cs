...
namespace WebMVC4.Controllers
{
    public class VideoController : Controller
    {
        ...
        public ActionResult Index(int VideoId = 0)
        {
            ...
            ViewBag.CateName = new CategoryBO().GetCategoryFull(CategoryId).Name;
            if (VideoId == 0)
            {
                var fi = new ContentBO().GetHotNews(CategoryId, 1).FirstOrDefault();
                model = new ContentBO().GetContentFull(int.Parse(fi.Id.ToString()));
            }
            else
            {
