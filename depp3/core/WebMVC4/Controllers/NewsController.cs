...
namespace WebMVC4.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        public ActionResult Relate(List<CONTENT_FULL> data, int CategoryId, string HeaderTitle = "Các tin khác")
        {
            if (data == null)
            {
                var PageSize = 10;
        ...
        public ActionResult RelateAuthor(int CategoryId, string username, string HeaderTitle = "Bài cùng tác giả")
        {
            var PageSize = 3;
            var data = new ContentBO().GetTopLastestContentFulls(PageSize, CategoryId, username);
            ViewBag.HeaderTitle = HeaderTitle;
            //ViewBag.PageNextShow = PageNextShow;
            //ViewBag.pageNext = pageNext;
            //ViewBag.CategoryId = CategoryId;
        ...
        public ActionResult TopViewNews(int CategoryId, int MaxLastestNews = 0, string Name = "Tin xem nhiều")
        {
        ...
        public ActionResult Index(int CategoryId, string CateName, int Page = 1, int Type = 0)
        {
            ...
            if (CategoryId == 40 || CategoryId == 54)
            ...
            if (!string.IsNullOrEmpty(cateobj.Param.MetaTitle))
            {
                siteTitle = cateobj.Param.MetaTitle;
            }

            if (!string.IsNullOrEmpty(cateobj.Param.MetaDesciption))
            {
                metaDescription = cateobj.Param.MetaDesciption;
            }
            ...
            if (CategoryId == 38 || CategoryId == 52)
            {
                lstNotId = "";
                PageSize = 3;
            }
            if (CategoryId == 81 || cateobj.ParentId == 81 || CategoryId == 87 || cateobj.ParentId == 87)
            {
                lstNotId = "";
                PageSize = 10;
            }
            ...
            if (CategoryId == 38 || CategoryId == 52)
            {
                ...
                foreach (var newsobj in articles)
                {
            ...
            if (CategoryId == 81 || CategoryId == 87)
            {
                return View("Index3", model);
            }
            if (cateobj.ParentId == 81 || cateobj.ParentId == 87)
            {
                var parent = new CategoryBO().GetCategoryFull(cateobj.ParentId.Value);
                ViewBag.PCateUrl = parent.Url;
                ViewBag.PCateName = parent.Name;
        ...
        public ActionResult Detail(int Id, string Title)
        {
            ...
            {
                siteTitle = newsobj.FileParam.MetaTitle;
            }

            if (!string.IsNullOrEmpty(newsobj.FileParam.MetaDesciption))
            {
                metaDescription = newsobj.FileParam.MetaDesciption;
            }
            ViewBag.Description = metaDescription;
            ViewBag.Title = siteTitle;
            if (newsobj.Type == 2)
            ...
            var parent = new CategoryBO().GetCategoryFull(newsobj.CateLiteObj.ParrentId);
            if(parent!=null)
            {
                ViewBag.PCateUrl = parent.Url;
                ViewBag.PCateName = parent.Name;
        ...
        public ActionResult LastestNews(string lang, int MaxLastestNews = 0)
        {
            ...
            var lstdata = new ContentBO().GetTopLastestContentFulls(MaxLastestNews, -1, "", lang);
