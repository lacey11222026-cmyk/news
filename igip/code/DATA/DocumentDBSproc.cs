...
namespace DATA
{
    public class DocumentDBSproc: DocumentDBBase
    {
        ...
        public override IEnumerable<Document> GetDocumentsSearch2(string keyword, string code, int agent, int area, int type, int categoryId, int status, int pageIndex, int pageSize, string fromdate, string todate, ref int totalRecords)
        {
            var select = "*";
            ...
            //{
            //    keyword = Utils.FormatKeywordSearch(agent);
            //    where += "( SignedByDesc LIKE N'%" + code + "%' ";
