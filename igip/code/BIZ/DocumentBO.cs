...
namespace BIZ
{
    public class DocumentBO
    {
        ...
        public List<DOCUMENT_FULL> GetDocumentsSearchPaged2(string title,string code, int agent, int area, int type, int categoryId, int status, int pageIndex, int pageSize, string fromdate, string todate, ref int totalRecords)
        {
            var Documents = DocumentDBBase.Create().GetDocumentsSearch2(title, code,agent,area,type,categoryId, status, pageIndex, pageSize, fromdate, todate, ref totalRecords);
            if (Documents == null)
                return null;
