...
namespace BIZ
{
    public class DocumentBO
    {
        ...
        public List<DOCUMENT_FULL> GetDocumentsSearchPaged(string title, int categoryId,int docType, int agent, int area, int type, int status, int pageIndex, int pageSize, string fromdate, string todate, ref int totalRecords,string fromdate2= "", string todate2 = "", int hit = -1)
        {
            var Documents = DocumentDBBase.Create().GetDocumentsSearch(title, categoryId, docType,agent, area,type, status, pageIndex, pageSize,fromdate,todate, fromdate2,todate2,hit, ref totalRecords);
            if (Documents == null)
                return null;
