...
namespace BIZ
{
    public class ContentBO
    {
        ...
        public string GetFilterContentsPaged_JSON(int pageIndex, int pageSize, string title, int categoryId, int status, string createdby, string fromdate = "", string todate = "",string lang="")
        {
            string keyCache = strGroupKeyCached + "_" + Constants.CACHE_KEY_ALL_CONTENTS_PAGED_JSON + pageIndex + pageSize + title + categoryId + status + createdby + fromdate + todate+lang;
            //string groupKeyCache = Constants.CACHE_GROUPKEY_CONTENT;
            ...
            int totalRecords = 0;
            List<CONTENT_FULL> contents = GetFilterContentFullsPaged(pageIndex, pageSize, title, categoryId, null, status, createdby, ref totalRecords, fromdate, todate,"","","",lang);
        ...
        private IEnumerable<Content> FilterContents(int pageIndex, int pageSize, string title, int categoryId, List<int> lstcate, int status, string createdby, ref int totalRecords, string fromdate = "", string todate = "", string lststatus = "", string alias = "", string orderBy = "PublishDate DESC",string lang="")
        {
            var contents = ContentDBBase.Create().GetFilterContents(pageIndex, pageSize, title, categoryId, lstcate, status, createdby, ref totalRecords, fromdate, todate, lststatus, alias, orderBy,lang);
            if (contents == null)
                return null;
            return contents;
        }
        ...
        public List<CONTENT_FULL> GetFilterContentFullsPaged(int pageIndex, int pageSize, string title, int categoryId, List<int> lstcate, int status, string createdby, ref int totalRecords, string fromdate = "", string todate = "", string lststatus = "", string alias = "", string orderBy = "PublishDate DESC",string lang="")
        {
            var contents = FilterContents(pageIndex, pageSize, title, categoryId, lstcate, status, createdby, ref totalRecords, fromdate, todate, lststatus, alias, orderBy,lang);
            if (contents == null)
        ...
        private IEnumerable<Content> GetTopLastestContents(int top, int categoryId, string username,string lang)
        {
            var result = ContentDBBase.Create().GetTopLastestContents(top, categoryId, username,lang);
            if (result == null)
                return null;
            return result;
        }
        private IEnumerable<Content> GetTopViewContents(int top, int categoryId,string lang, string title, string fromdate = "", string todate = "")
        {
            var result = ContentDBBase.Create().GetTopViewContents(top, categoryId, lang,title, fromdate, todate);
            if (result == null)
                return null;
            return result;
        }
        ...
        private List<CONTENT_FULL> GetHotNewsPrivate(int categoryId, int maxLastestNews)
        {
            ...
            var lstdata = new ContentBO().GetTopLastestContentFulls(maxLastestNews, categoryId,"", lang).ToList();
        ...
        public List<CONTENT_FULL> GetTopViewContentFulls(int top, int categoryId = 0,string lang="", string title = "", string fromdate = "", string todate = "")
        {
            var strKeyCached = strGroupKeyCached + "_" + Constants.CACHE_KEY_TOP_VIEW_CONTENTS + top + "_category" + categoryId + "_title" + title + "_fr" + fromdate + "_t" + todate + "_l" + lang;
            ...
            var lstItemBase = GetTopViewContents(top, categoryId,lang, title, fromdate, todate);
            if (lstItemBase == null || lstItemBase.Count() == 0)
                return null;
        ...
        public List<CONTENT_FULL> GetTopLastestContentFulls(int top, int categoryId = 0, string username = "",string lang="")
        {
            var strKeyCached = strGroupKeyCached + "_" + Constants.CACHE_KEY_TOP_LASTEST_CONTENTS + top + "_category" + categoryId + "_site" + username + "_lang" + lang;       
;
            ...
            var lstItemBase = GetTopLastestContents(top, categoryId, username,lang);
            if (lstItemBase == null || lstItemBase.Count() == 0)
                return null;
