...
namespace DATA
{
    public class DocumentDBSproc: DocumentDBBase
    {
        public override int CreateUpdateDocument ( Document Document )
        {
            try
            {
                ...
                using ( ShopOnlineDataContext datacontext = DataContext )
                    return datacontext.sp_Document_InsertUpdate(_id, _categoryid, _name, _categoryPathway, _description, _createdBy, _publishDate, _createdDate, _effectiveDate, _expiryDate, _status, _hits, _filepath, _code, _signedBy, _signedByDesc, _Area, _Agent, _Type,_DocType);
        ...
        public override IEnumerable<Document> GetDocumentsSearch(string keyword, int categoryId, int doctype, int agent, int area, int type, int status, int pageIndex, int pageSize, string fromdate, string todate, string fromdate2, string todate2, int hit, ref int totalRecords)
        {
            var select = "*";
            ...
            if (!string.IsNullOrEmpty(fromdate2) || !string.IsNullOrEmpty(todate2))
            {
                var culture = new CultureInfo("fr-FR", true);
                var _FormDate = new DateTime(1900, 1, 1);
                var _ToDate = new DateTime(9999, 1, 1);
                if (!string.IsNullOrEmpty(fromdate2))
                    _FormDate = DateTime.Parse(fromdate2, culture).Date;
                if (!string.IsNullOrEmpty(todate))
                    _ToDate = DateTime.Parse(todate2, culture).Date.AddDays(1).AddSeconds(-1);

                if (!string.IsNullOrEmpty(where))
                    where += " AND ";
                where +=
                    " (convert(nvarchar(23),EffectiveDate,121) between '" + _FormDate.ToString("yyyy-MM-dd HH:mm:ss.fff") + "' and '" + _ToDate.ToString("yyyy-MM-dd HH:mm:ss.fff") + "')";
