...
namespace DATA
{
    public abstract class DocumentDBBase: ShopOnlineDBBase
    {
        ...
        public abstract IEnumerable<Document> GetDocumentsSearch(string keyword, int categoryId,int doctype,int agent,int area,int type, int status,
                                                                 int pageIndex, int pageSize, string fromdate,
                                                                 string todate,  string fromdate2, string todate2, int hit ,ref int totalRecords);
        #endregion
