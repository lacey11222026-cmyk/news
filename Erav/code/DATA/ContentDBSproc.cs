...
namespace DATA
{
    public class ContentDBSproc : ContentDBBase
    {
        ...
        public override IEnumerable<Content> GetTopContentByIds(string ids, int top)
        {
            ...
            var select = " Id,Title,PublishDate,IntroText,Image,Url,CategoryId,SiteId,SiteUrl,Alias,Status";
            if (top > 0)
                select = "TOP(" + top + ") Id,Title,PublishDate,IntroText,Image,Url,CategoryId,Alias,Status";
            var where = "Id IN (" + ids + ") ";
            var orderBy = "PublishDate DESC";
        ...
        public override IEnumerable<Content> GetTopLastestContents(int top, int categoryId,string site,string lang)
        {
            ...
            if (top >= 1)
                select = "TOP(" + top + ") Id,Title,CreatedDate,PublishDate,IntroText,Image,Url,[Contents],[Type],CategoryId,Alias";
            var where = "Status = 4";
            if (categoryId > 0)
            {
                if (!string.IsNullOrEmpty(where))
