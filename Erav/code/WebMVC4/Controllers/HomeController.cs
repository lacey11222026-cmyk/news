...
namespace WebMVC4.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Intro2(int CategoryId)
        {
            var intro = new CategoryBO().GetCategoryFull(CategoryId);
        ...
        public ActionResult BannerRight(int top = 0, string cssClass = "", string lang = "vi-vn")
        {
            ...
            //    lstBanner = new BannerBO().GetTopLastestBanners(top, 2, 1);
            //else
            //    lstBanner = new BannerBO().GetTopLastestBanners(top, 12, 1);
        ...
        public ActionResult Banner(string lang = "vi-vn")
        {
            ...
            //else
            //    lstBanner = new BannerBO().GetTopLastestBanners(0, 11, 1);
            lstBanner = new BannerBO().GetTopLastestBanners(0, 1, 1);
            return PartialView(lstBanner);
        }
        //[OutputCache(Duration = 60, VaryByParam = "*", VaryByCustom = "browser")]
        ...
        {
            var lstid = new SystemConfigBO().GetValueByKey("HotNewsForCate_1001");
        ...
        public ActionResult HomeVideo(int CategoryId, string CateName)
        {
            ...
            var lstdata = new ContentBO().GetHotNews(CategoryId, 3);
        ...
        public ActionResult Slide()
        {
            ...
            var lstTopViewNews = new ContentBO().GetTopViewContentFulls(14,0,"","","","");
        ...
        private DateTime getdate(string input)
        {
            ...
            var result = new DateTime(int.Parse(arr[0]), int.Parse(month), int.Parse(arr[2]));
            return result;
        }
        private void ConvertLicens()
        {
            var lstdata = LicensingDAL.GetTopLastestDocuments(2);
            foreach (var item in lstdata)
            {
                var doc = new DOCUMENT_FULL();
                doc.CreatedBy = "quantri";
                doc.DocType = 2;
                doc.Name = $"Giấy phép số {item.LicenseNo}";
                doc.Code = item.LicenseNo;
                doc.CategoryId = 68;
                doc.CategoryPathway = ",3,68,";
                doc.CreatedDate = DateTime.Now;

                //ngày cấp phép
                doc.PublishDate = item.DateofIssue;
                doc.EffectiveDate = item.ExpiredLicensing;
                doc.SignedBy = "";
                doc.SignedByDesc = item.OrgName;
                doc.Description = item.Address;
                doc.Hits = 1;
                if(item.ExpiredLicensing< DateTime.Now)
                {
                    doc.Hits = 0;
                }    
                doc.Type = item.LicensingStatus;
                doc.Status = 1;
                doc.Agent = 0;
                doc.Area = item.AreaId;
                doc.ExpiryDate = Utils.ConvertToDate("01/01/9999", "dd-MM-yyyy");


                //file
                var strBuilder = new StringBuilder();
                strBuilder.Append(Request.PhysicalApplicationPath).Append("Files\\");
                var file = DownloadImage(strBuilder.ToString(), "http://www.erav.vn" + item.AttachmentLicensing, Utils.ConvertToRewriteLink(item.OrgName));
        ...
        private void ConvertDoc()
        {
            ...
            foreach (var item in lstdata)
            {
...

                {
                    ...
                    doc.CreatedBy = "quantri";
                    doc.DocType = 1;
                    doc.Name = item.OfficialName;
                    doc.Code = item.NoCode;
                    //category
                    if (item.CateOfficialID == 64)
                    {
                        doc.CategoryId = 81;
                        doc.CategoryPathway = ",69,81,";
                    }
                    if (item.CateOfficialID == 65)
                    {
                        doc.CategoryId = 82;
                        doc.CategoryPathway = ",69,82,";
                    }
                    if (item.CateOfficialID == 0)
                    {
                        doc.CategoryId = 83;
                        doc.CategoryPathway = ",69,83,";
                    }
                    doc.CreatedDate = DateTime.Now;

                    //ngày ban hành
                    doc.PublishDate = item.DatePublic;
                    ...
                    {
                        doc.EffectiveDate = item.EffectDate;
                    }
                    ...
                    {
                        doc.ExpiryDate = item.ExpiredDate;
                    }
                    doc.SignedBy = item.Writer;
                    doc.SignedByDesc = item.ID.ToString();
                    ...
                    {
                        doc.Description = Utils.RemoveAllHtmlTags(item.Quote);
                    }

                    doc.Hits = 1;//còn hiệu lực
                    if (item.ExpiredDate != null)
                    {
                        if (item.ExpiredDate.GetValueOrDefault() <= DateTime.Now)
                            doc.Hits = 0;
                    }
                    //if (item.EffectDate != null)
                    //{
                    //    if (item.EffectDate.GetValueOrDefault() > DateTime.Now)
                    //        doc.Hits = 2;
                    //}
                    doc.Type = item.DocTypeId;
                    doc.Status = 1;
                    doc.Agent = item.OrgId;
                    doc.Area = item.AreaId;
                    doc.FilePath = "";
                    ...
                    //file
                    var lstfile = OfficialDAL.GetFile(item.ID);
                    if (lstfile.Count > 0)
                    {
                        if (lstfile.Exists(x => x.FileName.ToLower().Contains(".pdf")))
                        {
                            var fileobj = lstfile.FirstOrDefault(x => x.FileName.ToLower().Contains(".pdf"));
                            var strBuilder = new StringBuilder();
                            strBuilder.Append(Request.PhysicalApplicationPath).Append("doc\\");
                            var file = DownloadImage2(strBuilder.ToString(), "http://www.erav.vn" + fileobj.FileName, Utils.ConvertToRewriteLink(fileobj.Title.ToLower()));
                        ...
                        {
                            var fileobj = lstfile.FirstOrDefault(x => x.FileName.ToLower().Contains(".doc"));
                            var strBuilder = new StringBuilder();
                            strBuilder.Append(Request.PhysicalApplicationPath).Append("doc\\");
                            var file = DownloadImage3(strBuilder.ToString(), "http://www.erav.vn" + fileobj.FileName, Utils.ConvertToRewriteLink(fileobj.Title.ToLower()));
        ...
        {
            //var imgName = Utils.ConvertToRewriteLink("Công ty TNHH Sản xuất và Thương mại Xuân Phát");
            //var strBuilder = new StringBuilder();
            //strBuilder.Append(Request.PhysicalApplicationPath).Append("Files\\");

            //DownloadImage(strBuilder.ToString(), "http://www.erav.vn"+ "/userfile/images/cmsfiles/capphep/Cong ty TNHH san xuat va thuong mai Xuan Phat 68.pdf", imgName);
            ...
            //if (request.Url.Host.Contains("vtk"))
            //{
        ...
        private string DownloadImage(string fromPath, string uri, string name)
        {
            ...
            try
            {
                if (!System.IO.File.Exists(fromPath + name + ".pdf"))
                    webClient.DownloadFile(uri, fromPath + name + ".pdf");
        ...
        private string DownloadImage2(string fromPath, string uri, string name)
        {
            ...
            try
            {
                if (!System.IO.File.Exists(fromPath + name ))
                    webClient.DownloadFile(uri, fromPath + name + ".pdf");
                return "/doc/" + name + ".pdf";
            }
            catch
        ...
        private string DownloadImage3(string fromPath, string uri, string name)
        {
            ...
            try
            {
                if (!System.IO.File.Exists(fromPath + name + ".doc"))
                    webClient.DownloadFile(uri, fromPath + name + ".doc");
                return "/doc/" + name + ".doc";
            }
            catch
            {
