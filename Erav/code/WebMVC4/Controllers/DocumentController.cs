...
namespace WebMVC4.Controllers
{
    public class DocumentController : Controller
    {
        ...
        public ActionResult Index(int Id, string fromdate = "", string todate = "", string keyword="",int agent=0,int area=0,int type=0, int Page = 1,int hit=-1)
        {
            ...
            lstcate = new CategoryBO().GetAllChildCategories(69, 10, false).Where(x => x.Published == 1).ToList();

            //var Model = new DocumentModel { listdata = data, pageIndex = Page, pageSize = PageSize, total = Total, CategoryId = Id, subcate = lstcate };

            data = new DocumentBO().GetDocumentsSearchPaged(keyword.Trim(), Id, 1, agent, area, type, 1, Page, PageSize, fromdate, todate, ref Total,"","",-1);
        ...
        public ActionResult Detail(int Id, string Title)
        {
            ...
            if (Title != Utils.ConvertToRewriteLink(newsobj.Name))
                return RedirectToAction("Document", "News", new { Id = Id, Title = Utils.ConvertToRewriteLink(newsobj.Name) });
        ...
        public ActionResult ViewFile(int Id)
        {
            ...
            if (newsobj == null || newsobj.Status != 1)
                return RedirectToAction("Error", "Home");
            ViewBag.FilePath = Config.Domain+newsobj.FilePath;
            ...
            {
                ViewBag.url = $"https://docs.google.com/gview?url={Domain}{newsobj.FilePath}&embedded=true";
            }
        ...
        public ActionResult DownloadFile(int Id)
        {
            ...
            //Action<int> addview = ViewAdd;
            //var asynSendNoti = addview.BeginInvoke(Id, null, null);
            if (string.IsNullOrEmpty(newsobj.FilePath))
