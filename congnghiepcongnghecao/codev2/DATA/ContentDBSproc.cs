...
namespace DATA
{
    public class ContentDBSproc : ContentDBBase
    {
        ...
        public override int CreateUpdateContent(Content content)
        {
            try
            {
                ...
                using (ShopOnlineDataContext datacontext = DataContext)
                    return datacontext.sp_Content_InsertUpdate(_id, _categoryid, _title, _categoryPathway, _alias, _introtext, _contents, _image, _thumbnail, _url, _createdby, _createddate, _publishDate, _status, _type, _hits, _params, _SiteId, IsHot, _SiteUrl);
        ...
        public override IEnumerable<Content> GetFilterContents(int pageIndex, int pageSize, string title, int categoryId, List<int> lstcate, int status, string createdby, ref int totalRecords,string fromdate="",string todate="", string lststatus = "",string alias="", string orderBy = "PublishDate DESC")
        {
        ...
        public override IEnumerable<Content> GetFilterContentsFrontend(int pageIndex, int pageSize, string title, int categoryId, ref int totalRecords, string fromdate, string todate, string lstNotId, string lang, int type,int site)
        {
            title = Utils.FormatKeywordSearch(title);
            var orderBy = "PublishDate DESC";
            var select = " Id,Title,PublishDate,IntroText,Image,Url,CategoryId,Hits,SiteId,SiteUrl";
            var where = "Status=4 ";
            //var orderBy = "Ordering ASC";
            //var orderBy = "PublishDate DESC";
            if (!string.IsNullOrEmpty(title))
        ...
        public override IEnumerable<Content> GetTopContentByIds(string ids, int top)
        {
            ...
            var select = " Id,Title,PublishDate,IntroText,Image,Url,CategoryId,SiteId,SiteUrl";
            if (top > 0)
        ...
        public override IEnumerable<Content> GetTopLastestContents(int top, int categoryId,int site)
        {
            var select = " Id,Title,PublishDate,CreatedDate,IntroText,Image,Url,[Contents],[Type],SiteId,SiteUrl,CategoryId";
            if (top >= 1)
                select = "TOP(" + top + ") Id,Title,CreatedDate,PublishDate,IntroText,Image,Url,[Contents],[Type],SiteId,SiteUrl,CategoryId";
            var where = "Status = 4";
            if (categoryId > 0)
            {
                if (!string.IsNullOrEmpty(where))
        ...
        public override IEnumerable<Content> GetTopViewContents(int top, int categoryId, string title, string fromdate, string todate)
        {
            title = Utils.FormatKeywordSearch(title);
