    ...
    public class QADBSproc : QADBBase
    {
        ...
        public override int CreateUpdateQA(QA manufactory)
        {
            ...
            try
            {
                ...
                {
                    datacontext.SP_QA_InsertUpdate(_id, _name, _email, _question, _answer, _order, _status, ref responecode);
        ...
        public override IEnumerable<QA> GetAllPaged(int pageIndex, int pageSize, ref int totalRecords, int? published,string lang)
        {
            ...
            if (!string.IsNullOrEmpty(lang))
            {
                if (!string.IsNullOrEmpty(where))
                    where += " AND ";
                lang = Utils.FormatKeywordSearch(lang);
                where += "( Name LIKE N'%" + lang + "%' ";
                where += "OR Email LIKE N'%" + lang + "%' ";
                
                where += "OR Question LIKE N'%" + lang + "%' )";
