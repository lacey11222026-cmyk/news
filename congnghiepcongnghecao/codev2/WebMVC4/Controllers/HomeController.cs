    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Slide()
        {
           ...
           var lstid = new SystemConfigBO().GetValueByKey("HotNewsForCate_"+Config.WebSite);
            ...
            var lstcontent = new ContentBO().GetTopLastestContentFulls(20, 0, Config.WebSite).Where(x => x.Type == 1).ToList();
        ...
        public ActionResult LastestNews2(string CateName, int CategoryId, int MaxLastestNews = 0)
        {
            ...
            var lstdata=new ContentBO().GetTopLastestContentFulls(30, CategoryId).Where(x=>x.CategoryId!=18).Take(MaxLastestNews).ToList();
        ...
        public ActionResult TopView(int Top)
        {
            var ToDate = DateTime.Now.ToString("dd/MM/yyyy");
            var  FromDate = DateTime.Now.AddMonths(-3).ToString("dd/MM/yyyy");
            var model = new ContentBO().GetTopViewContentFulls(Top ,0, "",FromDate,ToDate);
            return PartialView(model);
        }
