...
namespace WebMVC4.Controllers
{
    public class VideoController : Controller
    {
        ...
        public ActionResult Index(int VideoId = 0)
        {
            ...
            if (VideoId == 0)
            {
                model = new ContentBO().GetHotNews(6,1).FirstOrDefault();
            }
            else
            {
            ...
            var asynSendNoti = addview.BeginInvoke(model.Id,model.CategoryId.GetValueOrDefault(), null, null);

            return View(model);
        }
        public ActionResult HotVideo(int VideoId)
        {
            var model = new ContentBO().GetHotNews(6, 5);
        ...
        public ActionResult LoadVideo(int VideoId, int page)
        {
            ...
            ViewBag.page = page;
            int total = 0;
            var lst = new ContentBO().GetPageContentFullsFrontend(page, pagesize, 6, ref total, "", "", "", Session["lstNotId"].ToString(), "", -1);
        ...
        public ActionResult Audio(int VideoId = 0)
        {
            ...
            if (VideoId == 0)
            {
                model = new ContentBO().GetTopLastestContentFulls(1, 6).FirstOrDefault();
            }
            else
            {
                model = new ContentBO().GetContentFull(VideoId);
            ...
            Action<long,int> addview = ViewAdd;
            var asynSendNoti = addview.BeginInvoke(model.Id, 6, null, null);
