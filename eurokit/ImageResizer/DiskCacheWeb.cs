...
namespace ImageResizer.Plugins.DiskCache
{
    ...
    /// </summary>
    public class DiskCacheWeb : IAsyncTyrantCache, ICache, IPlugin, IIssueProvider, ILoggerProvider
    {
        ...
        public DiskCacheWeb(string virtualDir){
        ...
        public IPlugin Install(Config c) {
            ...
            if (c.get("diskcache.logging", false)) {
                if (c.Plugins.LogManager != null) 
                    log = c.Plugins.LogManager.GetLogger("ImageResizer.Plugins.DiskCacheWeb");
                else 
                    c.Plugins.LoggingAvailable += delegate(ILogManager mgr) {
                        if (log != null) log = c.Plugins.LogManager.GetLogger("ImageResizer.Plugins.DiskCacheWeb");
                    };
            }

            bool? inAsyncMode = c.Pipeline.UsingAsyncMode;
            if (inAsyncMode == null) throw new InvalidOperationException("You must set Config.Current.Pipeline.UsingAsyncMode before installing DiskCacheWeb");
