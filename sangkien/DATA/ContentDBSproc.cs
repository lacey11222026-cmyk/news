...
namespace DATA
{
    public class ContentDBSproc : ContentDBBase
    {
        ...
        public override IEnumerable<Content> GetFilterContentsFrontend(int pageIndex, int pageSize, string title, int categoryId, ref int totalRecords, string fromdate, string todate, string lstNotId, string lang, int type,int site)
        {
            title = Utils.FormatKeywordSearch(title);
            var orderBy = "PublishDate DESC";
            var select = " Id,Title,PublishDate,IntroText,Image,Url,CategoryId,Hits,SiteId,SiteUrl,Alias";
            var where = "Status=4 ";
            //var orderBy = "Ordering ASC";
            //var orderBy = "PublishDate DESC";
            if (!string.IsNullOrEmpty(title))
                where += "AND (Title LIKE N'%" + title + "%'  OR SiteUrl LIKE N'%" + title + "%')";
            if (categoryId > 0)
            {
        ...
        public override IEnumerable<Content> GetTopContentByIds(string ids, int top)
        {
            ...
            var select = " Id,Title,PublishDate,IntroText,Image,Url,CategoryId,SiteId,SiteUrl,Alias,IsHot,Hits";
            if (top > 0)
                select = "TOP(" + top + ") Id,Title,PublishDate,IntroText,Image,Url,CategoryId,Alias,IsHot,Hits";
        ...
        {
            var select = " Id,Title,PublishDate,CreatedDate,IntroText,Image,Url,[Contents],[Type],SiteId,SiteUrl,CategoryId,Alias,IsHot,Hits";
            if (top >= 1)
                select = "TOP(" + top + ") Id,Title,CreatedDate,PublishDate,IntroText,Image,Url,[Contents],[Type],CategoryId,Alias,IsHot,Hits";
            var where = "Status = 4";
            if (categoryId > 0)
            {
                if (!string.IsNullOrEmpty(where))
