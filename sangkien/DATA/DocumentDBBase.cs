...
namespace DATA
{
    public abstract class DocumentDBBase: ShopOnlineDBBase
    {
        ...
        public abstract IEnumerable<Document>  GetDocumentsSearch2(string keyword, string code, int agent, int area, int type, int categoryId, int doctype, int status, int pageIndex, int pageSize, string fromdate, string todate, ref int totalRecords);
        public abstract IEnumerable<Document> GetDocumentsSearch(string keyword, int categoryId,int doctype, int status,
