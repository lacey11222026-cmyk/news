    ...
    public class QADBSproc : QADBBase
    {
        ...
        public override int CreateUpdateQA(QA manufactory)
        {
            ...
            try
            {
                ...
                {
                    datacontext.SP_QA_InsertUpdate(_id, _name, _categoryid, _categoryPathway,_question, _answer, _order, _status, _PublishDate, ref responecode);
        ...
        public override IEnumerable<QA> GetTopLastest(int top,int categoryId)
        {
            var select = " *";
            if (top > 1)
                select = "TOP(" + top + ") *";
            ...
            var orderBy = "[PublishDate] DESC, Id DESC";

            return GetQAsDyn(select, where, orderBy);
        }
        public override IEnumerable<QA> GetAllPaged(int pageIndex, int pageSize, ref int totalRecords, int? published,int categoryId)
        {
