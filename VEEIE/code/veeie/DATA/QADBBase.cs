...
namespace DATA
{
    public abstract class QADBBase: ShopOnlineDBBase
    {
        ...
        public abstract IEnumerable<QA> GetAllPaged(int pageIndex, int pageSize, ref int totalRecords, int? published,int categoryId);
        public abstract IEnumerable<QA> GetQAsDyn ( string select, string where, string orderBy );
        public abstract  IEnumerable<QA> GetTopLastest(int top, int categoryI);
