...
namespace WebMVC4.Controllers
{
    public class IntroController : Controller
    {
        ...
        [LocalizationActionFilter]
        public ActionResult Question(int CategoryId,string CateName, int Page = 1,int Id=0)
        {
            var cateobj = new CategoryBO().GetCategoryFull(CategoryId);
            if (cateobj == null)
                return RedirectToAction("Error", "Home");
            if (CateName != Utils.ConvertToRewriteLink(cateobj.Name))
                return RedirectToAction("Question", "Intro", new { CategoryId = CategoryId, CateName = Utils.ConvertToRewriteLink(cateobj.Name) });
            ...
            var PageSize = 30;
            int Total = 0;
            var albums = new QABO().GetQAsPaged( Page, PageSize, ref Total,1, CategoryId);
