...
{
    [Authorize(Roles = "Administrator,NewsEdit,NewsPublish,Guest")]
    public class AdminNews2Controller : Controller
    {
        ...
        public ActionResult DetailCrawl(int Id,int Type)
        {
            ...
            if (HttpContext.User.IsInRole("Administrator") || HttpContext.User.IsInRole("Guest"))
        ...
        public ActionResult GetNewsDetail(int Id = 0)
        {
            ...
            var model = new CONTENT_FULL { PublishDate = DateTime.Now,IntroText=" ", Id = 0, Alias = HttpContext.User.Identity.Name, CategoryId = 0, Hits = 0, CreatedBy = HttpContext.User.Identity.Name, FileParam = new List<FileInfo>() };
            ViewBag.UserName = HttpContext.User.Identity.Name;
            var lstdata = Membership.GetAllUsers();
            //ExHandler.Handle(new Exception(), "User", "User" + lstdata.Count);
            List<AccountInfo> lstuser = new List<AccountInfo>();
            ...
            if (Id > 0)
            {
                ...
                try
                {
                    model.FileParam = JsonConvert.DeserializeObject<List<FileInfo>>(model.Thumbnail);
                }
                catch
                {

                    model.FileParam = new List<FileInfo>();
                }
                if (model.FileParam == null)
                {
                    model.FileParam = new List<FileInfo>();
