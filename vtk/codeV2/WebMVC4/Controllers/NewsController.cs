...
namespace WebMVC4.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        public ActionResult Index(int CategoryId, string CateName, int Page = 1, int Type = 0)
        {
            ...
            try
            {
                 lstHotNews = new ContentBO().GetHotNews(CategoryId, PageSize);
            ...
            try
            {
                if (cateobj.ParentId == 6)
                {
                    if (Page == 1)
                    {
                        model.articles = new ContentBO().GetPageContentFullsFrontend(Page, PageSize, CategoryId, ref Total, "", "", "", "");
                    }
                    else
                    {
                        model.hotnews = new ContentBO().GetPageContentFullsFrontend(Page - 1, PageSize, CategoryId, ref Total, "", "", "", lstNotId);
                        Total++;
                    }
            ...
            if (cateobj.ParentId == 6)
            {
                model.listcate = new CategoryBO().GetAllChildCategories(cateobj.ParentId.GetValueOrDefault(), 10, false);
        ...
        public ActionResult Search(string q = "", string fromdate = "", string todate = "", int categoryId = -1, int Page = 1)
        {
            ...
            int total = 0;
            var articles = new ContentBO().GetPageContentFullsFrontend(Page, pageSize, 6, ref total, fromdate, todate, q, "","",-1,Config.WebSite);
        ...
        public ActionResult Preview(int Id)
        {
            ...
            try
            {
                newsobj.FileParam = JsonConvert.DeserializeObject<List<FileInfo>>(newsobj.Thumbnail);
            }
            catch
            {

                newsobj.FileParam = new List<FileInfo>();
            }
            ...
            //}
            ViewBag.Cate = new CategoryBO().GetCategoryFull(newsobj.CategoryId.GetValueOrDefault());
