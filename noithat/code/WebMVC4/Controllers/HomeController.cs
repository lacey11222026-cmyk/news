...
namespace WebMVC4.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var lstcategory = new CategoryBO().GetAllCategoryFullsByPosition(UTILS.Constants.CategoryPosition.TopMenu, 18, false);

            return PartialView(lstcategory);
        }
        public ActionResult SubMenuProduct(int CategoryId)
        {
            var lstcategory = new CategoryBO().GetAllChildCategories(CategoryId, 18, false);
            if (lstcategory != null)
            {
        ...
        public ActionResult SubMenuProductMobile(int CategoryId)
        {
            var lstcategory = new CategoryBO().GetAllChildCategories(CategoryId, 18, false);
            if (lstcategory != null)
            {
        ...
        public ActionResult SubMenu(int CategoryId)
        {
            var lstcategory = new CategoryBO().GetAllChildCategories(CategoryId, 18, false);
            if(lstcategory!=null)
            {
        ...
        public ActionResult SubMenu2(int CategoryId)
        {
            var lstcategory = new CategoryBO().GetAllChildCategories(CategoryId, 18, false);
            if (lstcategory != null)
            {
        ...
        public ActionResult Menu()
        {
            ...
            lstcategory = lstcategory.Where(x => x.ParentId == 0).ToList();
            foreach(var category in lstcategory)
            {
                category.Param = JsonConvert.DeserializeObject<CategoryParam>(category.Params);
            }    
        ...
        {
            var lstcategory = new CategoryBO().GetAllCategoryFullsByPosition(UTILS.Constants.CategoryPosition.TopMenu, 113,false);
            lstcategory = lstcategory.Where(x => x.ParentId == 0).ToList();
            foreach (var category in lstcategory)
            {
                category.Param = JsonConvert.DeserializeObject<CategoryParam>(category.Params);
        ...
        public ActionResult LastestNews2(string CateName, int CategoryId, bool IsMobile = false, int MaxLastestNews = 0)
        {
            if (MaxLastestNews == 0)
            {
                MaxLastestNews = Convert.ToInt32(ConfigurationManager.AppSettings["MaxLastestNews"]);
        ...
        {
            var lstcate = new CategoryBO().GetAllCategoryFullsByPosition(UTILS.Constants.CategoryPosition.MainMenu, 10,false);
            return PartialView(lstcate);
        }
        //[OutputCache(Duration = 120, VaryByParam = "*", VaryByCustom = "browser")]
        [ChildActionOnly]
        public ActionResult TopGroup2()
        {
            //var lstcate = new CategoryBO().GetAllChildCategories(7,4,false);
            var lstcategory = new CategoryBO().GetAllCategoryFullsByPosition(UTILS.Constants.CategoryPosition.MainMenu, 28, false);
            return PartialView(lstcategory.Where(x=>x.ParentId==7||x.ParentId==29).ToList());
        }
        #endregion
        //[Authorize]
