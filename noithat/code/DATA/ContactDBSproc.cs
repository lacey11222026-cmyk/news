...
namespace DATA
{
    public class ContactDBSproc : ContactDBBase
    {
        ...
        public override IEnumerable<Contact> GetAllContacts(int categoryId,int status,string name)
        {
            var select = "*";
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                where += "( Name Like '%" + name + "%'  Or Mobile Like '%" + name + "%' ) ";
            }
            return GetContactsDyn(select, where, orderBy);
        }
        public override IEnumerable<Contact> GetContacts(string lstContactIds) {
            var where = "Id IN (" + lstContactIds + ")";
            var select = "*";
            string orderBy = "Id ASC";
            return GetContactsDyn(select, where, orderBy);
        }
