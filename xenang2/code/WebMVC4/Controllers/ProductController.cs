...
namespace WebMVC4.Controllers
{
    public class ProductController : Controller
    {
        ...
        public ActionResult Index(int CategoryId, string CateName, int ManuId = 0, int Page = 1, int type = 0,int orderType=0,int w=0)
        {
            ...
            ViewBag.canonical = "https://xenangdienkomatsu.vn"+cateobj.Url;

           
            ViewBag.Keywords =  ConfigurationManager.AppSettings["DefMetaKeyword"];
            
            ViewBag.SiteTitle = cateobj.Name;
            ViewBag.SiteDescription = cateobj.Description;
            if (!string.IsNullOrEmpty(cateobj.Param.SiteTitle))
            {
                ViewBag.SiteTitle = cateobj.Param.SiteTitle;

            }
            if (!string.IsNullOrEmpty(cateobj.Param.SiteDescription))
            {
                ViewBag.SiteDescription = cateobj.Param.SiteDescription;
            ...
            if (Page > 1)
            {
                ViewBag.SiteTitle = ViewBag.SiteTitle + " Trang-" + Page.ToString();
            ...
            var model = new ProductModel { Cate = cateobj, listdata = data, pageIndex = Page, pageSize = PageSize, total = Total, CategoryId = CategoryId, ManufactoryId = ManuId, listmanu = lstManu };
            ...
            var _staticCategoryList = new CategoryBO().GetAllCategoriesFull(UTILS.Constants.CategoryType.Product);
           model.listcate = new CategoryBO().GetCategoryByUserName(_staticCategoryList, "quantri",
                                                                               true);
            return View(model);
        }
        [LocalizationActionFilter]
        ...
        public ActionResult Detail(int Id, string Title)
        {
            ...
            if (manuobj == null)
            {
                manuobj = new MANUFACTORY_FULL { Id = -1, Title = "" };
            }    
                //return RedirectToAction("Error", "Home");
        ...
        [OutputCache(Duration = 120, VaryByParam = "*", VaryByCustom = "browser")]
        public ActionResult TopProduct2(int top, int categoryId, int manufactoryId, bool? ishot, bool? isnew)
        {
            ...
            if (categoryId > 0)
            {
                ...
                model.listcate = new CategoryBO().GetAllChildCategories(categoryId,10,false);
                model.Cate = cateobj;
        ...
        public ActionResult TopManu(int CategoryId, int ManuId,string link)
        {
            ...
            var listAll= new ManufactoryBO().GetAllManufactoryFulls(-1); ;
            if (CategoryId == 0)
            {
                data = listAll;
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        data.Add(new MANUFACTORY_FULL { Id = item.ManufactoryId.GetValueOrDefault(), Title = item.ManufactoryName ,Image= listAll.FirstOrDefault(x=>x.Id==item.ManufactoryId).Image});
                    }
                }
            }
            ViewBag.ManuId = ManuId;
        ...
        public ActionResult RelateProduct(int id, int categoryId, int manuid)
        {
            try
            {
                var lstdata = new ProductBO().GetTopProduct(8, categoryId, -1, 1, null, null, "");
