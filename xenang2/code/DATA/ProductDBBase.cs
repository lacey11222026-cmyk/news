...
namespace DATA
{
    public abstract class ProductDBBase: ShopOnlineDBBase
    {
        ...
        public abstract IEnumerable<Product> GetAllPaged(string keyword, int categoryId, int manufactoryId,int pageIndex, int pageSize, ref int totalRecords, int? published, bool? isHot, bool? isNew, string lang = "", double min = 0, double max = 0, int orderType = 0);
