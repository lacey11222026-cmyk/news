    ...
    public class CategoryBO
    {
        ...
        public List<CATEGORY_FULL> GetCategoryByUserName(List<CATEGORY_FULL> staticCategoryList, string username, bool isAdmin)
        {
            ...
            if (isAdmin)
            {
                ...
                foreach (var item in lstcate)
                {
                    if (item.ParentId > 0)
                    {
                        var x1 = new CATEGORY_FULL { Id = item.Id, ParentId = item.ParentId, Name = item.Name,Pathway=item.Pathway ,Type=item.Type,Link=item.Link};
                        if (item.NodeLevel != 0)
                        {
        ...
        public CATEGORY_FULL GetCategoryFull(int categoryId)
        {
            try
            {
                ...
                var strKeyCached = Constants.CACHE_KEY_CATEGORY + categoryId;
        ...
        public List<CATEGORY_FULL> GetAllCategoriesFull(UTILS.Constants.CategoryType categoryType, string lang = "")
        {
            try
            {

                var strKeyCached = Constants.CACHE_KEY_ALL_CATEGORIES + (int)categoryType + "_lang" + lang;
        ...
        public List<CATEGORY_FULL> GetAllCategoriesFullPaged(int pageIndex, int pageSize, ref int totalRecords)
        {
            try
            {
                var strKeyCached = Constants.CACHE_KEY_ALL_CATEGORIES_PAGED + pageIndex + pageSize;
        ...
        {
            var strKeyCached = Constants.CACHE_KEY_ALL_CHILDCATEGORIES + categoryId + "_numbertak" + numberTake + "_" + isRandom.ToString();
        ...
        public List<CATEGORY_FULL> GetAllHomepageCategoryFulls()
        {
            try
            {
                ...
                var strKeyCached = Constants.CACHE_KEY_ALL_HOMEPAGE_CATEGORIES;
        ...
        {
            var strKeyCached = Constants.CACHE_KEY_ALL_CATEGORIES_BY_POSITION + (byte)categoryPosition;
        ...
        {
            var strKeyCached = Constants.CACHE_KEY_CATEGORY + categoryFull.Id;
